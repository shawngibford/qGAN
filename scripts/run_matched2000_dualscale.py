...
The matched-2000ep COUNTERPART of ``run_dualscale_fidelity.py``
...
audited ``run_dualscale_fidelity.py`` (D-11-10: ``core.eval`` is
...
Key contracts:
    ...
    D-11-10 — ``core.eval`` reused UNCHANGED (imported, never edited);
              ``git diff --stat core/`` must be empty.
...
Usage
-----
    python -m run_matched2000_dualscale \\
        [--out results] [--csv-path ./data.csv]

Writes ``results/matched2000_dualscale.json`` (long-form dual-scale
...
# core.eval helpers — REUSE UNCHANGED (D-11-10). Import only; the
...
MATCHED2000_RUNS_REL = Path("results/matched2000/runs")
...
def _run_base(model_kind: str, pipeline: str, seed: int) -> Path:
    ...
    layout: ``results/matched2000/runs/<model>/<seed>``. ``pipeline``
    ...
    for fname in ("samples.npy", "inverse_kwargs.npz"):
        if not (base / fname).exists():
            raise FileNotFoundError(
                ...
                f"14-02 sweep harness (run_matched2000_sweep.sh) to "
...
def _headline_rows() -> list:
    ...
    (already computed by 14-02 via ``core.eval``; this driver does NOT
...
def emit_rows(real_OD_flat, real_windowed_OD, real_log_delta) -> list:
    ...
    reconstruct OD from the saved samples, emit every ``core.eval``
...
def build_aggregates(rows: list) -> list:
    ...
    These are aggregations of ``core.eval`` outputs (mean/std via the
...
def main() -> None:
    """CLI entry point — emit results/matched2000_dualscale.json."""
    ap = argparse.ArgumentParser(
        description=(
            "Phase 14 plan 14-08 matched-2000ep dual-scale re-emit "
            "(reuses core.eval UNCHANGED — D-11-10; no "
    ...
    obj = {
        ...
        "metric_helpers": {
            "functions": [
                "core.eval.compute_emd",
                "core.eval.compute_moments",
                "core.eval.compute_acf",
                "core.eval.compute_dtw",
            ],
