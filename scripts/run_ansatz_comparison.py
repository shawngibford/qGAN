...
``results/ansatz_comparison.json`` cleanly.
...
computed via ``core.eval.full_metric_suite`` UNCHANGED (D-10-20).
...
Variant provenance (D-13-01, D-10-15):
    * V1 (depth-4, range, 75 params) — REUSED, NOT recomputed-by-training. Its
      ...
      ``results/transform_ablation/runs/B/{42..46}`` are read and the
    ...
    * V2 (depth-8, range, 135 params) — NEW Phase-13 runs under
      ``results/ansatz/runs/V2/{42..46}``.
    * V3 (depth-4, linear, 75 params) — NEW Phase-13 runs under
      ``results/ansatz/runs/V3/{42..46}``.
...
Usage
-----
    python -m run_ansatz_comparison \\
        [--out results] [--csv-path ./data.csv] \\
        [--ansatz-root results/ansatz]

Writes ``results/ansatz_comparison.json``.
"""
...
def _run_base(variant: str, seed: int, ansatz_root: Path) -> Path:
    ...
    if variant == "V1":
        return repo / "results" / "transform_ablation" / "runs" / "B" / str(seed)
...
def _rows_for_run(
    ...
    Metric math is ``core.eval.full_metric_suite`` UNCHANGED
    (D-10-20).
    """
    out = []
    # ── scale = OD ───────────────────────────────────────────────────────────
...
def main() -> None:
    """CLI entry point — emit results/ansatz_comparison.json."""
    ap = argparse.ArgumentParser(
        description=(
            "Phase 13 ARCH-02 ansatz comparison aggregator "
            "(reuses core.eval.full_metric_suite UNCHANGED — "
    ...
    obj = {
        ...
        "metric_helpers": {
            "functions": ["core.eval.full_metric_suite"],
