...
Usage
-----
    ./qgan_env/bin/python -m run_matched2000 \\
...
def build_dataset_for_pipeline(csv_path: Path) -> DatasetBundle:
    ...
    from core import BATCH_SIZE, WINDOW_LENGTH
...
def generate_wgan_samples(generator, n: int, seed: int):
    ...
    from core import BATCH_SIZE, NOISE_HIGH, NOISE_LOW, NUM_QUBITS
...
def _train_quantum(model: str, bundle: DatasetBundle, epochs: int,
    ...
    # mutate core/training.py — byte-unchanged discipline).
...
def _train_vae(bundle: DatasetBundle, epochs: int, seed: int) -> tuple:
    ...
    from core.models.nonadversarial import VAEBaseline
...
def _train_ar(bundle: DatasetBundle, seed: int) -> tuple:
    ...
    from core.models.nonadversarial import ARBaseline
...
def main() -> None:
    ...
    ap.add_argument(
        "--out-root",
        type=Path,
        default=Path("results/matched2000"),
