...
safe by plan 14-09 SUMMARY for the analogous ``core.models.quantum``
...
``parameter_count`` recorded in ``results/model_info.json`` for the
same kind. Every mismatch is collected into a list and reported in a single
loud raise BEFORE the JSON is written.

D-14-22 byte-freeze: ``core/`` is opened ONLY via the public model-
class imports. ``git diff --stat core/`` MUST remain empty across
...
# only reason this script needs the path tweak (so the public `core`
...
# ─────────────────────────────────────────────────────────────────────────────
from core.models.classical import (  # noqa: E402
    WGANMLPGenerator,
    WGANCNNGenerator,
    WGANLSTMGenerator,
)
from core.models.nonadversarial import (  # noqa: E402
    VAEBaseline,
    ARBaseline,
)
from core.models.critic import Critic  # noqa: E402
...
def _wgan_mlp_layers() -> list[dict]:
    ...
    Slice indices verified against ``core/models/classical.py:58-95``.
    """
    return [
        {
            "layer_type": "Linear",
...
def _wgan_cnn_layers() -> list[dict]:
    ...
    Slice indices verified against ``core/models/classical.py:98-141``.
    """
    return [
        {
            "layer_type": "Reshape",
...
def _wgan_lstm_layers() -> list[dict]:
    ...
    Slice indices verified against ``core/models/classical.py:144-212``.
    """
    return [
        {
            "layer_type": "Reshape",
...
def _vae_split_encoder_decoder(specs: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    ``core/models/nonadversarial.py:67-73``."""
...
def _drift_check(
    ...
    if mismatches:
        raise AssertionError(
            ...
            "total_params disagree with results/model_info.json "
...
def main() -> None:
    ...
    ar_spec = {
        ...
        "burn_in_steps": 50,
        "source_path": "core/models/nonadversarial.py:139-158",
    }
    ...
    out = {
        ...
        "models": {
            ...
            "wgan_cnn": {
                ...
                "source_path": "core/models/classical.py:98-141",
                "carve_strategy": (
                    "single nn.Parameter (params_pqc) sliced into per-layer "
                    "weight/bias views applied via torch.nn.functional"
                ),
            ...
            "wgan_lstm": {
                ...
                "source_path": "core/models/classical.py:144-212",
                "carve_strategy": (
            ...
            "vae": {
                ...
                "carve_strategy": (
                    ...
                    "dec_out); ELBO objective trained in run_baselines.py"
                ),
                "encoder": vae_enc,
                "decoder": vae_dec,
                "objective": "ELBO",
        ...
        "consumed_artifacts": {
            "results/model_info.json": model_info.get("data_hash"),
        },
        "drift_gate": (
