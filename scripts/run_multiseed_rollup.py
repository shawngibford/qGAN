...
and emits `results/multiseed_summary.json` with `{mean, std, n,
seeds}` per cell plus a provenance header.

NO new training, NO new seeds, NO device, NO model, NO torch, NO pennylane,
NO `core` model code (D-12-03 / D-10-13). Single invocation, stdlib
only — `statistics.fmean`/`statistics.stdev` (RESEARCH Open Q3: zero new
dependency; pandas/numpy are NOT required).

`core/` is NEVER modified by this driver (D-10-13 invariant).

Pattern source: `run_utility.py:38-58` (verbatim repo-root resolver +
