...
(``results/matched2000/runs/<model>/<seed>/`` +
``results/headline_canonical.json``), a COMPLETE per-model +
cross-model + analysis figure suite. Every figure is written as a
``<stem>.png`` + ``<stem>.pdf`` + ``<stem>.json`` triple so each manuscript
figure is traceable to a ``results/*`` value (T-14-11). There is NO
...
Idiom mirrors ``run_introspect_figures.py`` end-to-end (the pattern
...
``reconstruct_od`` logic of ``run_dualscale_fidelity.py:194-236``
...
Usage::
    ...
    python -m run_figure_suite [--figures-dir DIR]
"""
...
def _bootstrap_repo_on_path() -> Path:
    ...
    ``python run_figure_suite.py`` does not put the repo root on
    ``sys.path`` (only the script's own dir). Walk up to the dir holding
    ``core/preprocessing.py`` and prepend it (verbatim with
    ``run_dualscale_fidelity.py:69-83`` so both ``-m`` and bare
...
DATA_CSV = "data.csv"
MATCHED2000_REL = Path("results/matched2000/runs")
HEADLINE_REL = Path("results/headline_canonical.json")
...
MATCHED2000_DUALSCALE_REL = Path("results/matched2000_dualscale.json")
...
CANONICAL_LOCK_REL = Path("results/canonical_config_lock.json")
...
def _require(path: Path, what: str) -> Path:
    ...
    (``run_matched2000_sweep.sh``) to (re)generate the sweep
...
def _save(fig: plt.Figure, figures_dir: Path, stem: str,
    ...
    manuscript figure is traceable to its ``results`` source.
    """
    written: list[Path] = []
    for ext in ("png", "pdf"):
...
def reconstruct_od(repo: Path, model: str, seed: int) -> np.ndarray:
    ...
    ``run_dualscale_fidelity.py:221-236`` — the
...
def render_cross_model_emd(repo: Path, figures_dir: Path) -> list[Path]:
    ...
    dual = _load_json(
        repo / "results/matched2000_dualscale.json", "dualscale"
    )
...
# Render-only. SOLE numeric source: results/matched2000_dualscale.json
...
def render_matched2000_dualscale_comparison_table(
    ...
    JSON via its ``results/*.json`` rglob — no verifier edit).
    ...
    lines.append(
        ...
        "`results/matched2000_dualscale.json` by "
        "`run_figure_suite.py` "
        ...
        "and passes `verify_number_provenance.py` unmodified."
    )
    lines.append("")
    lines.append(
...
def render_existing_introspection(figures_dir: Path) -> list[Path]:
    ...
    from run_introspect_figures import (
...
# core/ remains byte-frozen (D-14-22). The frozen-checkpoint
...
def render_training_convergence_all_models(repo: Path,
    ...
    companion = {
        ...
        "render_only": True,
        "source_artifacts": [
            f"results/matched2000/runs/<{','.join(ADVERSARIAL_MODELS_WITH_EMD_AVG)}>/"
...
def render_tstr_crossmodel(repo: Path, figures_dir: Path) -> list[Path]:
    ...
    Render-only over ``results/tstr.json`` (the SOLE source for
    ...
    """
    src_path = repo / Path("results/tstr.json")
...
def render_tstr_crossmodel_matched2000(repo: Path,
    ...
    Render-only over ``results/tstr_matched2000.json`` (the 14-20
    ...
    ``results/matched2000/runs/``). The four quantum variants
    ...
    for safety.
    """
    src_path = repo / Path("results/tstr_matched2000.json")
    ...
    companion = {
        ...
        "render_only": True,
        "source_artifact": "results/tstr_matched2000.json",
        "matched_budget": True,
...
def render_param_efficiency_pareto(repo: Path,
    ...
    """
    mi_path = repo / Path("results/model_info.json")
...
def render_noise_robustness_quantum(repo: Path,
    ...
    Render-only over ``results/noise_model_sensitivity.json``
    ...
    explicit note.
    """
    src_path = repo / Path("results/noise_model_sensitivity.json")
...
def render_shot_noise_robustness(repo: Path,
    ...
    Render-only over ``results/shot_noise_sensitivity.json``
    ...
    """
    src_path = repo / Path("results/shot_noise_sensitivity.json")
...
def main() -> None:
    ...
    # could appear (D-14-10). core/ stays byte-frozen (D-14-22).
