...
`revision/_build_baseline_notebook.py`, trains the `TSTRLiteLSTM` soft-sensor
(also verbatim, D-11-10: NOT promoted to `revision/core/`), and writes two
long-form JSON artifacts:

  * `revision/results/tstr.json`        — EVAL-01 (R2/MAE/RMSE + Phase-10 anchor)
  * `revision/results/augmentation.json` — EVAL-04 (mixing-ratio lift curve)

Pattern source: `revision/run_baselines.py` (CLI/driver shape) +
`revision/_build_baseline_notebook.py:167-501` (recon + TSTR logic, copied
...
`revision/core/` is NEVER modified by this driver (D-11-10 / D-10-13 invariant).
"""
