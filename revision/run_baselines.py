...
Mirrors ``revision/run_ablation.py`` structure (same windowed data + the same
...
Three model families (Phase 10 CONTEXT decisions):
    ...
    D-10-08 — WGAN variants train via ``train_wgan_gp`` UNCHANGED, with the
              ...
              ``revision.core`` (never hardcoded literals).
    ...
    D-10-20/24 — NO new metric helpers here; per-run fidelity metrics are
              computed in the Wave-4 notebook via ``revision.core.eval``.
...
Usage
-----
    python -m revision.run_baselines --model {wgan_mlp|wgan_cnn|wgan_lstm|vae|ar} \\
        --pipeline {A|B} --seed N [--epochs M] \\
        [--out-root revision/results/baselines] [--csv-path ./data.csv]
...
from revision.core.models.nonadversarial import ARBaseline, VAEBaseline
...
def main() -> None:
    ...
    ap.add_argument(
        "--out-root",
        type=Path,
        default=Path("revision/results/baselines"),
