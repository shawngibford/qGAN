...
> config-lock JSONs under `revision/results/` (canonical, default_75, v1, v2, v3).
> Re-run `revision/run_circuit_diagrams.py` to (re)generate the locks AND the figures
> together; `revision/verify_number_provenance.py --target revision/docs/circuit_atlas.md`
> is the executable gate that proves every literal here resolves to a
> `revision/results/*.json` value (success criterion 5). All five circuit diagrams are
...
- **`V1` / `V2` / `V3`** — the matched-budget ansatz variants from
  `revision/run_matched2000.py` (Plan 14-02), each trained fresh at the matched
...
Source: `revision/results/default_75_config_lock.json`
...
Source: `revision/results/canonical_config_lock.json`
...
Source: `revision/results/v1_config_lock.json`
(`source_path`: `revision/run_matched2000.py:118`).
...
in `revision/run_matched2000.py`, while `default_75` is the architectural label for the
...
Source: `revision/results/v2_config_lock.json`
...
Source: `revision/results/v3_config_lock.json`
(`source_path`: `revision/run_matched2000.py:122`).
...
- All five circuit diagrams were emitted by `revision/run_circuit_diagrams.py`
  using `qml.draw_mpl(qnode, style="pennylane")` — never a bespoke matplotlib
  gate drawing.
- Gated by `./qgan_env/bin/python revision/verify_number_provenance.py --target revision/docs/circuit_atlas.md`.
