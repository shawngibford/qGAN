...
> in-repo `main (4) copy.tex` / `supp_material.tex` are READ-ONLY reference
...
> in this file resolves to a value in some `revision/results/*.json` and is
> proven by `revision/verify_number_provenance.py --target
> revision/docs/paper_blocks_refs_methods.md`. The authoritative source for any
...
> per-comment point-by-point rebuttal lives in `revision/docs/reviewer_response.md`.
...
`main (4) copy.tex` — location-independent (A1).
...
> `\cite{}` keys are identified from `main (4) copy.tex` and are the load-bearing
> handle for each fix.
...
**Anchor (`main (4) copy.tex` §1.4 line ~92 and §2.4 line ~151), before:**
...
**Anchor (`main (4) copy.tex` §2.4 line ~148, quantum kernels sentence), before:**
...
**Anchor (`main (4) copy.tex` §1.3 line ~84), before:**
...
**Anchor (`main (4) copy.tex` §1.3 line ~86), before:**
...
**Anchor (`main (4) copy.tex` §3.1 line ~172), before:**
...
> — see `revision/results/model_info.json` `dataset.window_length`,
...
**Insertion sentence — keyed to the §1.4 transition anchor (`main (4) copy.tex`
...
> opening (`main (4) copy.tex` line ~146-148) where QML is introduced; one
...
regenerated ACF figures are `revision/results/figures/acf_*.png` whose
companion JSON carries the corrected `xlabel`). In the manuscript the relevant
caption is `\label{fig:acf}` (`main (4) copy.tex` line ~251).
...
> `revision/results/figures/acf_iqp_sel_55_repro.png` (PAPER-09 / Plan 04).

### PAPER-11 / R1-m7 item 2 — Missing space: "Figure A5).This" → "Figure A5). This"

**Anchor (`main (4) copy.tex` §4.2 line ~261), before:**
...
> main-text usage `LUCY\textregistered` at `main (4) copy.tex` line ~178. The
...
**Anchor (`main (4) copy.tex` §3.2 line ~178), before (a literal
...
**Anchor (`main (4) copy.tex` abstract line ~49), before:**
...
**Anchors:** `main (4) copy.tex` abstract line ~49 ("bio-manufacturing") and
...
**Anchor (`main (4) copy.tex` §5 Concluding Remarks line ~296), before:**
...
**Anchor (`main (4) copy.tex` `\label{fig:acf}` caption line ~250, and
...
`\label{fig:cdf}`, `\label{fig:qq}`, `\label{fig:acf}` (`main (4) copy.tex`
...
% The regenerated high-DPI sources (revision/results/figures/*.pdf, dpi=150,
