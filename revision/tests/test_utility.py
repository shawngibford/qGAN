...
pytest checks so any future regression is caught loudly:
  ...
  - ``revision/core/`` byte-untouched (D-11-10, T-11-04)
...
Convention mirrors ``revision/tests/test_classical.py`` /
...
``python revision/tests/test_utility.py`` script (the existing files rely on
...
revision.* package and qgan_env libs):
    pytest revision/tests/ -q
or as a plain script:
    ./qgan_env/bin/python revision/tests/test_utility.py
"""
...
from revision.run_utility import (  # noqa: E402
    EXPECTED_DATA_HASH,
    _compute_data_hash,
    reconstruct_od,
)
...
_QB42_BASE = (
    REPO / "revision" / "results" / "transform_ablation" / "runs" / "B" / "42"
)
...
def test_phase10_reproduction_recompute():
    ...
    OD-EMD live via the verbatim reconstruct_od + revision.core.eval.compute_emd
