...
# Part 2 - gen and critic models
    ####################################################################################
    #
    # count the parameters - REVERTED to complex circuit with ZZ interactions
    #
    ####################################################################################
    def count_params(self):
        # Complex circuit parameter counting (reverted):
    ...
    ####################################################################################
    #
    # Complex quantum generator with IQP encoding + strongly entangled layers (REVERTED)
    #
    ####################################################################################
    def define_generator_circuit(self, noise_params, params_pqc):
        # Apply noise encoding layer first (RZ rotations)
        self.encoding_layer(noise_params)
        
        # index for the parameter tensor
        idx = 0
        
        # Hadamard initialization for all qubits (like original TensorFlow implementation)
        ...
        # Apply L layers of the enhanced circuit
        ...
        # Final measurement preparation rotations
        for qubit in range(self.num_qubits):
            if idx < len(params_pqc):
                qml.RX(phi=params_pqc[idx], wires=qubit)
                idx += 1
...
# Calculate expected parameters for verification - REVERTED to complex circuit
...
print("🚀 REVERTED: COMPLEX IQP + STRONGLY ENTANGLED QUANTUM GAN")
print("=" * 60)
