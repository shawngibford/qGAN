"""Generate 06_baseline_comparison.ipynb from inlined cell sources.
...
Mirrors _build_analysis_notebook.py (the 09.1 generator pattern).
...
* reconstruct_od copied VERBATIM from _build_analysis_notebook.py:95-149,
  ...
  results/baselines/runs/<model>/<pipeline>/<seed>/ AND the reused
  09.1 quantum layout results/transform_ablation/runs/<pipeline>/<seed>/.
* Metrics computed ONLY via core.eval (D-10-20) — no new helpers.
...
NB_PATH = Path("06_baseline_comparison.ipynb")
...
**D-10-20** every fidelity metric is computed via `core.eval` only (no
new helpers). Per **D-10-13** the TSTR-lite forecaster is copied verbatim from
the 09.1 notebook generator and is NOT promoted to `core/`.
"""))
...
for m in NEW_MODELS:
    for p in PIPELINES:
        for s in SEEDS:
            cfg_path = Path(f"results/baselines/runs/{m}/{p}/{s}/config.yaml")
...
`results/transform_ablation/runs/<pipeline>/<seed>` and the 5 new
models at `results/baselines/runs/<model>/<pipeline>/<seed>`
...
# Cell 6 — long-form metrics rows via core.eval ONLY (D-10-20).
# Mirrors _build_analysis_notebook.py:159-209 (same metric suite + DTW recipe).
CELLS.append(md("""\
## Long-form metric rows — `core.eval` ONLY (D-10-20)

For every (model_kind × pipeline × seed) reconstruct OD and compute the locked
metric suite using **only** `core.eval` helpers (D-10-20 — no new
...
def _read_new_cfg(model_kind: str) -> dict:
    ...
    return yaml.safe_load(
        Path(f"results/baselines/runs/{model_kind}/A/42/config.yaml").read_text()
    )
...
print("wrote results/baseline_comparison.json "
...
(R²computed inline). These are NOT promoted to `core/` (D-10-13).
"""))
CELLS.append(code("""\
