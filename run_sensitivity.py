...
``results/transform_ablation/runs/<pipeline>/<seed>/``). Phase 12 is
...
the *unchanged* fidelity suite (``core.eval.full_metric_suite``,
...
Locked decisions honored here:
  ...
  * **D-10-13** — ``core/`` is byte-untouched. The noisy circuit body
...
Usage
-----
    python run_sensitivity.py --pipeline {A|B} --seed N \\
        --condition <one of the 11 tokens> \\
        [--out-root results/sensitivity] [--csv-path ./data.csv]
...
# run_utility.py:42-57. Anchor EVERY artifact path at REPO.
# ─────────────────────────────────────────────────────────────────────────────
...
from core.eval import full_metric_suite  # noqa: E402
...
# run_ablation.py:195-208 (Pitfall 3 — the ``* 0.1`` scaling and
...
# run_utility.py:144-179 (Pitfall 4 — the Pipeline-B
...
# (core/models/quantum.py:122-171) reproduced in THIS driver — not in
...
# headline JSONs anchored at REPO/results:
#
...
def aggregate(out_root: Path) -> tuple[Path, Path]:
    ...
    REPO/results. Idempotent — fully recomputed from the per-cell
...
def main() -> None:
    ...
    _ck_path = (
        REPO / "results" / "transform_ablation" / "runs"
