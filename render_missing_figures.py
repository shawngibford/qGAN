...
Render-only. Produces:
  ...
  4. preprocessing_ablation_comparison — 2×2 grouped bar chart comparing
     ...
     TSTR-lite R². Source: results/transform_ablation/metrics.csv
...
Usage:
    python -m render_missing_figures [--figures-dir DIR]
"""
...
from core.data import load_and_preprocess  # noqa: E402
...
def render_cross_model_dtw_dualscale(repo: Path, figures_dir: Path) -> list[Path]:
    dual = _load_json(repo / "results/matched2000_dualscale.json")
    ...
    companion = {
        ...
        "source": (
            "results/matched2000_dualscale.json#rows filtered to "
...
def render_cross_model_acf_overlay(repo: Path, figures_dir: Path) -> list[Path]:
    ...
    companion = {
        ...
        "source": (
            ...
            "data.csv via core.data.load_and_preprocess()['log_delta']"
        ),
...
def render_preprocessing_pipeline_4panel(repo: Path, figures_dir: Path) -> list[Path]:
    ...
    run_ablation.py:134-152 (pipeline == 'B' branch).
    """
    bundle = load_and_preprocess(csv_path=str(repo / "data.csv"))
    ...
    companion = {
        ...
        "source": (
            "core/data.load_and_preprocess(csv_path='data.csv') "
...
def render_preprocessing_ablation_comparison(
    ...
    metrics. Source: results/transform_ablation/metrics.csv (per-seed
    ...
    metrics_csv = repo / "results/transform_ablation/metrics.csv"
    seed_spread = _load_json(
        repo / "results/transform_ablation/seed_spread.json"
    )
    ...
    for ax, (title, metric_name, scale, ref, direction) in zip(flat_axes, metric_specs):
        ...
        if metric_name == "tstr_lite_r2" and not present:
            tstr_path = repo / "results/transform_ablation/tstr_lite.json"
            try:
    ...
    companion = {
        ...
        "source": (
            "results/transform_ablation/metrics.csv (long-form, "
