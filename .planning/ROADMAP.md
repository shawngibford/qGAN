...
- [ ] 14-16-PLAN.md — R3 forensic remediation: fix R3-CR-2 log-return scale mismatch (`revision/run_matched2000_dualscale.py:368-372` — standardized synth vs unnormalized real, 50× inflation) by switching to `norm_log_delta` and re-emitting `matched2000_dualscale.json`; fix R3-CR-1 histogram-density structural bias (`revision/run_distribution_emd.py:94-141` — `density=True` per-distribution renormalization rewards posterior-collapse and out-of-range distributions, inverts quantum rankings) by switching to shared-edges-from-real-range with disclosed out-of-range fake mass and re-emitting `distribution_emd.json`; update reviewer_response.md + methods_full.md + reconciliation_note.md with the corrected numbers and the strong-claim parametric-efficiency-equivalence framing (`55 quantum params statistically-equivalent to 10⁴–10⁵ classical params on OD-EMD, Welch p > 0.36 \\|d\\| ≤ 0.65 n=5; quantum significantly beats every WGAN on LR-EMD p ≤ 0.014`); re-render `cross_model_emd` + `qq_overlay` against corrected aggregates with CR-1 sha256-seeded determinism preserved; add peer_review_remediation.md r3-follow-up section enumerating both bugs and their remediation; passes v2.1 gate on all 10 paper-facing docs (supports PAPER-01/02/08/09 + INFRA-03)
...
| 14. Paper Revision & Release Freeze | v2.0 | 14/16 | In Progress|  |
