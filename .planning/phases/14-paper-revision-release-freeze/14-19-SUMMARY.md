...
provides:
  ...
  - "Atomic .gitignore: results/ exclusion + !revision/results/ negations"
...
tech-stack:
  added:
    - "fastdtw==0.3.4 (pinned — used by revision/core/eval.py DTW)"
...
`./qgan_env/bin/python revision/verify_freeze_ready.py` was run against this committed HEAD and confirms it evaluates the committed tree:
...
- `(d)` release.md — **expected FAIL**: `revision/docs/release.md` is plan 14-07's deliverable; the assertion fails until 14-07 authors it. This is the intended ordering guard.
...
1. **Canonical CSVs** — `real.csv`/`fake.csv` reverted to HEAD; the working-tree drift was unintended and the HEAD versions are consistent with the certified `revision/results/*.json`.
...
- **requirements-pinned.txt** — added `fastdtw==0.3.4` (used by `revision/core/eval.py` DTW) and `pandas<3.0` (pandas 3.0 breaks statsmodels). Header comments preserved.
- **.gitignore atomic commit** — fixed the ` *.csv` leading-space typo back to `# *.csv` (CSVs stay tracked); the `results/` exclusion and BOTH `!revision/results/` negations land in this one commit (T-14-28). `git check-ignore revision/results/*.json` against the committed `.gitignore` returns empty — no provenance JSON ignored.
- **Baseline artifacts tracked** — 50 `metrics.json` + 50 `config.yaml` under `revision/results/baselines/runs/` force-staged with `git add -f`. The 50 `*.npy`, 60 `*.npz`, 40 `*.pt` large binaries are NOT committed (D-14-21 / Agent 6 HIGH-1).
...
- Added `.gitignore` patterns for the large baseline binaries (`revision/results/baselines/runs/**/*.npy`, `**/*.npz`, `**/*.pt`) so the 150 large binaries do not break the clean-tree assertion.
...
- **Files modified:** `revision/verify_freeze_ready.py`
- **Commit:** `6518323`
...
- **revision/docs/release.md** — correctly absent; it is plan 14-07's deliverable. The hardened gate's `(d)` assertion fails until 14-07 runs (intended ordering guard). Not created by this plan.

## Boundary Compliance

No tag was cut, no Zenodo deposit performed, `revision/core/` untouched (byte-frozen D-11-10), and no numbers recomputed or retrained. This plan only established the freeze pre-conditions and hardened the gate.

## Self-Check: PASSED

- `LICENSE` present and tracked — FOUND
- `revision/requirements-pinned.txt` contains `fastdtw` + `pandas<3.0` — FOUND
- Committed `.gitignore` contains `!revision/results/`; `git check-ignore` empty for provenance JSON — VERIFIED
- 50 `metrics.json` + 50 `config.yaml` tracked under `revision/results/baselines/runs/` — FOUND
