...
- `verify_number_provenance.py` (RESEARCH Pattern 3): scrubs identifier digits (D-14-NN / R1-MN / phase / line citations) then extracts every numeric literal and asserts each resolves at stated precision to a `results/*.json` value (raw + re-serialized text, with float-precision fallback). Unresolved literal → explicit `raise AssertionError` + non-zero exit (`python -O` safe). Self-verified: both regenerated docs PASS (17 + 5 distinct literals all resolve); negative test (injected `999987.654321`) exits 1.
...
- `run_model_info.py` — pure-aggregator emitter: model_info.json + reconciliation_note.md + JSON→markdown render path (796 lines)
...
- `verify_number_provenance.py` — reusable RESEARCH-Pattern-3 number-provenance gate (201 lines, explicit-raise)
...
- **Files modified:** `run_model_info.py`
...
## Self-Check: PASSED
- `run_model_info.py` — FOUND (796 lines, pure aggregator, yaml.safe_load)
...
- `verify_number_provenance.py` — FOUND (201 lines, raise AssertionError, non-zero exit on unresolved; both docs PASS; negative test exits 1)
