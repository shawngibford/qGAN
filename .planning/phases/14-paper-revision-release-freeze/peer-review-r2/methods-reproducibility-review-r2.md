...
**PASS-WITH-FINDINGS.** Plan 14-13 unambiguously closes the two BLOCKERs and HIGH-1/HIGH-2/HIGH-3 from the round-1 methods review. A third-party reviewer who clones this repo, installs `revision/requirements-pinned.txt`, and follows the rerun template in `methods_full.md §5.2` can (a) install a working environment that matches the captured framework versions exactly, (b) load the headline checkpoint and re-evaluate `iqp_sel_55_headline`, (c) rerun any matched-2000ep training and pass the strict-accept gate, (d) reproduce reported numbers to ~1e-6 EMD on the same CPU+BLAS stack. The remaining gaps are well-disclosed (no Zenodo DOI yet — Phase 14-07 still open; no `torch.use_deterministic_algorithms`; per-epoch trajectories not committed) rather than hidden.
...
| Install pinned environment | **BLOCKER-1** (only `>=` constraints) | **PASS** — `revision/requirements-pinned.txt` with `==` pins committed (commit `4ea576b`) |
| Load headline checkpoint | **BLOCKER-2** (`*.pt` in `.gitignore`, no exception) | **PASS** — `revision/checkpoints/best_checkpoint.pt` (~6 MB) is tracked via `.gitignore` exception line 70: `!revision/checkpoints/best_checkpoint.pt` |
...
1. **Checkpoint integrity.** `shasum -a 256 revision/checkpoints/best_checkpoint.pt` → `f7cceb52285f753b9f5f697086f3042817761d37f3112a9b36dc580ebe03b082`. This matches `canonical_config_lock.json#checkpoint_sha256` exactly. Reviewer-side integrity-check passes.

2. **Checkpoint loadability.** `torch.load('revision/checkpoints/best_checkpoint.pt', map_location='cpu', weights_only=False)` succeeds and yields a `dict` with the expected eight keys. `ckpt['epoch']` = 1969, `ckpt['emd']` = 0.08384301715430653, `ckpt['params_pqc'].shape` = `torch.Size([55])`, `dtype` = `torch.float32`, `ckpt['mu']` = 0.0024553430266678333, `ckpt['sigma']` = 0.021407155320048332. All match `canonical_config_lock.json` to all digits.

3. **PyPI availability of pinned versions.** Queried `https://pypi.org/pypi/{pkg}/{version}/json` for every package in `revision/requirements-pinned.txt`. All six pins are present on PyPI and resolve: pennylane 0.43.0 (requires Python ≥3.11 — satisfied by 3.11.14), torch 2.9.0, numpy 2.3.4, scipy 1.16.2, matplotlib 3.10.7, PyYAML 6.0.3.

4. **`.gitignore` checkpoint exception.** Lines 36-37 of `.gitignore` ignore `*.pth` / `*.pt`; lines 69-70 add the explicit exception `!revision/checkpoints/best_checkpoint.pt` under a "Phase 14 plan 14-13" header comment. The negation works as intended (file is tracked, working tree clean).
...
|---|---|---|
| `revision/results/model_info.json` | YES | Top-level field + per-model field for all 10 models |
| `revision/results/methods_full.json` | YES | Top-level `buckets.5_reproducibility.data_hash` |
| `revision/results/framework_versions.json` | YES | Top-level (HIGH-2 fix) |
| `revision/results/classical_architectures.json` | YES | (HIGH-2 fix) |
| `revision/results/figures/circuits/iqp_sel_55.json` | YES | (HIGH-2 fix — per-circuit JSON) |
| `revision/results/figures/circuits/default_75.json` | YES | (HIGH-2 fix) |
| `revision/results/figures/circuits/V1.json` | YES | (HIGH-2 fix) |
| `revision/results/figures/circuits/V2.json` | YES | (HIGH-2 fix) |
| `revision/results/figures/circuits/V3.json` | YES | (HIGH-2 fix) |
| `revision/results/headline_canonical.json` | YES | Pre-existing |
| `revision/results/matched2000_dualscale.json` | YES | Pre-existing |
| `revision/results/multiseed_summary.json` | YES | Pre-existing |
| `revision/results/tstr.json` | YES | Pre-existing |
| `revision/results/predictive_discriminative.json` | YES | Pre-existing |
| `revision/results/augmentation.json` | YES | Pre-existing |
| `revision/results/baseline_comparison.json` | YES | Pre-existing |
| `revision/results/baseline_classical_wgan.json` | YES | Pre-existing |
| `revision/results/baseline_nonadversarial.json` | YES | Pre-existing |
| `revision/results/fidelity_dualscale.json` | YES | Pre-existing |
| `revision/results/total_adversarial_param_budget.json` | YES | New (Plan 14-13 T3) |
| `revision/results/reconciliation_deltas.json` | YES | New (Plan 14-13 T3) |

### JSONs WITHOUT `data_hash`

I separately grepped every top-level JSON in `revision/results/*.json` and every figure companion JSON in `revision/results/figures/*.json`:
...
From `revision/docs/methods_full.md` §4.2 (lines 380-397, byte-identical to `revision/docs/reviewer_response.md` lines 188-206):
...
`revision/requirements-pinned.txt` exists (commit `4ea576b`) and contains exact `==` pins for every package listed in `framework_versions.json`:
...
Python version is recorded as a comment (`# python==3.11.14`) rather than a pin (because pip can't pin python itself from a requirements file). This is the right convention. The install snippet at the top of the file is correct: `python -m venv qgan_env && pip install -r revision/requirements-pinned.txt`. PyPI verification confirms all six versions resolve. `methods_full.md §4.1` references the pinned file by path. RESOLVED.

**REPRO-LOW-2 (minor):** the pinned file does not list `statsmodels` (which `revision/core/eval.py` imports via `statsmodels.tsa.stattools.acf`). `statsmodels` is an indirect dependency that pip will pull, but for strict resolution it should be in the pinned file. Round-1 reviewer also didn't catch this. Not a blocker.

### METHODS-BLOCKER-2 — `best_checkpoint.pt` gitignored → **RESOLVED**

`revision/checkpoints/best_checkpoint.pt` (6,036,217 bytes) is git-tracked via `.gitignore` exception lines 69-70. Working tree is clean against HEAD. `sha256` matches `canonical_config_lock.json#checkpoint_sha256` at `f7cceb52…`. `torch.load(...)` returns a `dict` with the expected eight keys and the expected scalar metadata (`epoch=1969`, `emd=0.08384…`). A reviewer can now recompute the headline EMD without obtaining the checkpoint out-of-band. RESOLVED.

### METHODS-HIGH-1 — "bit-identical" overclaim → **RESOLVED**

`revision/docs/methods_full.md §5.1` (lines 418-426) now reads:

> The same seed produces trajectories that agree to ~1e-6 EMD on the same CPU+BLAS+pinned-pip-freeze stack (`revision/requirements-pinned.txt`); bit-determinism would require `torch.use_deterministic_algorithms(True)` which is not set in the byte-frozen `revision/core/training.py` (D-14-22). The pinned-env + tracked-checkpoint contract (`revision/checkpoints/best_checkpoint.pt`, sha256 = `f7cceb52…` per `canonical_config_lock.json#checkpoint_sha256`) delivers reproducibility-within-numerical-tolerance, not bit-determinism (Plan 14-13, METHODS-HIGH-1 remediation).

This is exactly the wording the round-1 review recommended (option a). The determinism contract is now (i) explicit about its scope (~1e-6 EMD agreement, not bit-identity), (ii) explicit about the missing piece (`torch.use_deterministic_algorithms(True)`), (iii) explicit about why the missing piece can't be added (`D-14-22` byte-freeze on `revision/core/training.py`), and (iv) explicit about what IS guaranteed (pinned-env + tracked-checkpoint). RESOLVED.
...
`revision/docs/training_protocol.md` row 34 is now split (lines 34-35):
...
1. **Set up environment:**
   ...
   ```
2. **Verify data integrity:** `python -c "import hashlib; import numpy as np; ... # hash matches 91e447d4624e25b3"` (the `_compute_data_hash` function in `revision/run_matched2000.py:242-252`).
3. **Re-evaluate the headline (no retrain):** `./qgan_env/bin/python -m revision.run_canonical_headline` — loads `revision/checkpoints/best_checkpoint.pt` and writes `revision/results/headline_canonical.json`.
...
6. **Re-emit aggregates and figures:** `revision/run_methods_full.py`, `revision/run_framework_versions.py`, `revision/run_classical_arch_extract.py`, `revision/run_model_info.py`, `revision/run_figure_suite.py`.
...
| Headline checkpoint (`iqp_sel_55_headline` epoch 1969 EMD 0.08384) | YES (load) | Tracked at `revision/checkpoints/best_checkpoint.pt`; SHA matches lock; load → params_pqc (55,). Reviewer can re-evaluate, not re-train. |
...
| Final samples (`samples.npy`) for each (model, seed) of the 45-run matrix | YES (load) | Tracked at `revision/results/matched2000/runs/<model>/<seed>/{samples.npy,metrics.json,config.yaml,inverse_kwargs.npz}`. The reviewer can either load them directly OR retrain and compare. |
...
| TimeGAN / TSTR downstream-model trained on synthetic samples | YES (load) | `revision/results/tstr.json` carries the TSTR scores; samples are loadable; the TSTR downstream training is deterministic given a seed. |
...
Location: `revision/docs/reviewer_response.md` line 49 (R1-m4 row of the Reviewer 1 Minor Issues table):
...
Location: `revision/docs/methods_full.md` lines 171-198. The §2.i VAE table reports 562 params; the §2.i prose does NOT explicitly say "the VAE is intentionally NOT parameter-matched to the quantum generator." The `revision/core/models/nonadversarial.py:11-13` source comment ("NOT parameter-matched to the quantum generator (D-10-03)") never made it into the methods doc.
...
`revision/core/eval.py` imports `statsmodels.tsa.stattools.acf`. `revision/requirements-pinned.txt` doesn't pin statsmodels. Pip will pull a transitive version via numpy/scipy resolution, but for strict reproducibility the explicit pin matters when statsmodels' ACF implementation has changed across versions. **Severity: LOW.** Pin should be added in a follow-up.
...
The two BLOCKERs from round-1 are decisively closed; the three HIGHs are closed; the determinism claim is now honest and properly scoped. A third-party AIChE reviewer with the pinned env and `revision/checkpoints/best_checkpoint.pt` can re-evaluate the headline EMD to all digits and rerun any matched-2000ep training to ~1e-6 EMD agreement. The CR-4 disclosure is honest on all six criteria the orchestrator brief listed.
...
- `/Users/shawngibford/dev/phd/qGAN/revision/requirements-pinned.txt`
...
- `/Users/shawngibford/dev/phd/qGAN/revision/results/framework_versions.json`
- `/Users/shawngibford/dev/phd/qGAN/revision/results/methods_full.json`
- `/Users/shawngibford/dev/phd/qGAN/revision/results/model_info.json`
- `/Users/shawngibford/dev/phd/qGAN/revision/results/classical_architectures.json`
- `/Users/shawngibford/dev/phd/qGAN/revision/results/figures/circuits/{default_75,iqp_sel_55,V1,V2,V3}.json`
- `/Users/shawngibford/dev/phd/qGAN/revision/docs/methods_full.md`
- `/Users/shawngibford/dev/phd/qGAN/revision/docs/training_protocol.md`
...
- `/Users/shawngibford/dev/phd/qGAN/revision/docs/completeness_sweep_manifest.md`
