...
- v2 source = `scripts/verify_number_provenance.py` HEAD (328 lines, `_SCHEMA = "v2 (Phase 14 plan 14-13 — boundary-strict resolution + render-only exclusion)"`).
...
| PROV-HIGH-2 (`data_hash` invariant inconsistency) | OPEN | **CLOSED** — `data_hash = "91e447d4624e25b3"` recorded in `circuit_diagrams.json`, `classical_architectures.json`, `framework_versions.json`; `EXPECTED_DATA_HASH` explicit-raise in `scripts/run_model_info.py` |
