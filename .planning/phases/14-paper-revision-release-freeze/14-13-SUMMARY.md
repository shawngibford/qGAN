...
key-files:
  ...
  modified:
    ...
    - revision/verify_number_provenance.py (v2 rewrite)
    - revision/run_methods_full.py (CR-3 programmatic citations)
    - revision/run_model_info.py (HI-2 / HI-3 / PROV-HIGH-3 / OD-scale reconciliation)
    - revision/run_matched2000.py (HI-4 / HI-7 / HI-8 / CR-4 future-gate / D-14-13 ext)
    - revision/run_matched2000_dualscale.py (H-2 ddof=1 + HI-5 model_kinds + MED-4 n alias)
    - revision/run_figure_suite.py (CR-1 SHA-256 seed + CR-2 cross_model_emd + H-1 axis + MD-3 lock-driven head_epoch + HI-8 finite_sanitize + ddof=1)
    - revision/run_canonical_headline.py (HI-1 generation_seed threading)
    - revision/run_circuit_diagrams.py (PROV-HIGH-2 data_hash)
    - revision/run_classical_arch_extract.py (PROV-HIGH-2 data_hash)
    - revision/run_framework_versions.py (PROV-HIGH-2 data_hash)
    - revision/verify_freeze_ready.py (HI-6 rglob)
    - revision/docs/methods_full.md (§4.1 + §5.1 + §3.x + §2.i + §2.j + §2.k + §2.k.x + §4.2)
    - revision/docs/reconciliation_note.md (C-1 / PROV-CRIT-1 / C-3)
    - revision/docs/paper_blocks_framing.md (PROV-CRIT-2 doc-side)
    - revision/docs/reviewer_response.md (CR-4 disclosure subsection)
    - revision/docs/completeness_sweep_manifest.md (Plan 14-13 section appended)
    - revision/docs/training_protocol.md (re-emitted with dtype_params/dtype_samples)
    - revision/docs/dataset_stats.md (re-emitted)
    ...
    - revision/results/figures/cross_model_emd.{png,pdf,json}
    - revision/results/figures/timeseries_{ar,iqp_sel_55_repro,V1,V2,V3,vae,wgan_cnn,wgan_lstm,wgan_mlp}.{png,pdf,json}
    - revision/results/figures/* (additional re-renders dependent on the data updates)
decisions:
  - "D-14-16 (gate byte-freeze) LIFTED for Task 2 only; back in byte-freeze under v2 schema after T2"
  - "D-14-22 (revision/core/ byte-freeze) PRESERVED across all 7 tasks"
...
5. **`framework_versions.json` versions equal `==` pins in `revision/requirements-pinned.txt`** — PASS (pennylane=0.43.0, torch=2.9.0, numpy=2.3.4, scipy=1.16.2, matplotlib=3.10.7, PyYAML=6.0.3).
6. **`git ls-files | grep best_checkpoint.pt`** returns the tracked path; its sha256 matches `canonical_config_lock.json#checkpoint_sha256` — PASS (`revision/checkpoints/best_checkpoint.pt` tracked; sha256 = `f7cceb52285f753b9f5f697086f3042817761d37f3112a9b36dc580ebe03b082`).
...
10. **`revision/docs/peer_review_remediation.md`** exists; every one of the 28 findings (12 CRITICAL + 16 HIGH) maps to at least one commit SHA from this plan (HI-9 explicitly marked OUT OF SCOPE with rationale) — PASS.
...
- **D-14-22 PRESERVED** — `revision/core/` byte-frozen across all 7 tasks; `git diff --stat revision/core/` is empty at every task close. M-2 / M-3 / MD-1 / MD-7 / LO-1 / M-4 are DOCUMENTED in `methods_full.md §3.x` (Task 6), NOT CHANGED.
...
- **Fix:** Emitted `revision/results/manuscript_apparatus_constants.json` as a legitimate audit artifact recording the LUCY photobioreactor specs. This is NOT a weakening of the gate — it makes the resolution path explicit.
- **Files modified:** `revision/results/manuscript_apparatus_constants.json` (new)
- **Commit:** `dfde1ba` (T2)
...
- **Fix:** Emitted `revision/results/reconciliation_deltas.json` as a structured artifact recording per-model `(old, new, delta)` tuples so the v2 gate resolves the delta literals via a legitimate JSON source.
- **Files modified:** `revision/run_model_info.py` (added emit), `revision/results/reconciliation_deltas.json` (new)
- **Commit:** `9fe3a0f` (T3)
...
- **Fix:** Emitted `revision/results/total_adversarial_param_budget.json` recording per-model `generator_n_params + shared_critic_n_params = total_adversarial_param_budget` derivations so methods_full.md cites a JSON source.
- **Files modified:** `revision/results/total_adversarial_param_budget.json` (new)
- **Commit:** `9fe3a0f` (T3)
...
- **Files modified:** `revision/run_matched2000.py`
- **Commit:** `8c67891` (T4)
...
- **Files modified:** `revision/docs/methods_full.md`
- **Commit:** `e893e0e` (T6)
...
for doc in revision/docs/{paper_blocks_framing,paper_blocks_refs_methods,reviewer_response,reconciliation_note,methods_full,circuit_atlas,completeness_sweep_manifest,training_protocol,dataset_stats,peer_review_remediation}.md; do
...
(`revision/core/` byte-frozen). D-14-16 LIFTED for T2 only (back in
