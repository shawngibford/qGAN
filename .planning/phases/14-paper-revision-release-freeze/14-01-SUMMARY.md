...
requires:
  - phase: 13-architecture-introspection
    provides: "_TOPOLOGIES config-selectable precedent in revision/core/models/quantum.py (the exact selector shape mirrored here)"
  - phase: 09.1-r1-m3-ablation
    provides: "revision/core/preprocessing.py Pipeline A/B/C definitions (used to pin the native headline pipeline)"
provides:
  - "Config-selectable NON-default 55-param IQP:SEL circuit (circuit_id=iqp_sel_55) in revision/core/models/quantum.py"
  - "revision/run_recover_canonical.py — checkpoint-driven recovery + D-14-07 phase-blocking equivalence gate driver"
  - "revision/results/canonical_recovery.json — decomposition + checkpoint provenance (epoch 1969, verbatim mu/sigma, sha256, optimizer breadcrumbs)"
  - "revision/results/canonical_config_lock.json — locked canonical config + pinned native pipeline B + checkpoint sha256"
...
- `revision/run_recover_canonical.py` - Checkpoint-driven recovery (`--recover-only`) + D-14-07 phase-blocking equivalence gate (`--assert-equivalence`); worktree-aware gitignored-checkpoint resolver
- `revision/results/canonical_recovery.json` - Decomposition, epoch 1969, verbatim mu/sigma, checkpoint sha256, optimizer LR/betas breadcrumbs, native pipeline B + evidence
- `revision/results/canonical_config_lock.json` - Locked `circuit_id=iqp_sel_55`, decomposition, pinned pipeline B, stored mu/sigma, checkpoint sha256, equivalence-gate status
- `revision/core/models/quantum.py` - `_CIRCUIT_IDS` selector + `circuit_id` arg + variant-aware param formula and Step-5 final-rotation block + `last_param_index()` structural-introspection helper; `default_75` branch literally byte-frozen
- `revision/tests/test_utility.py` - `test_core_untouched` rescoped (Rule-1) to the real T-14-02 default-path-byte-freeze invariant
...
- **Found during:** Task 2 (full `pytest revision/tests/` regression run)
- **Issue:** `revision/tests/test_utility.py::test_core_untouched` is a Phase-11-era guard (D-11-10) asserting *zero* git diff under `revision/core/`. Phase-14 D-14-01 explicitly mandates adding the recovered 55-param circuit to `revision/core/models/quantum.py`, so the old guard would fail a correct, equivalence-gated deliverable. The true invariant is **default-path byte-freeze** (T-14-02), not whole-core-freeze (same rescope Phase-13 ARCH-01 applied to its core guard).
- **Fix:** Rescoped the test to assert (a) `revision/core/__init__.py` (the frozen architecture-constants baseline) has zero git diff, and (b) a no-arg `QuantumGenerator()` still builds the byte-identical 75-param `default_75` circuit with the expected forward-output shape.
- **Files modified:** `revision/tests/test_utility.py`
- **Verification:** `pytest revision/tests/test_utility.py::test_core_untouched` passes; default-path forward pass proven bit-identical vs HEAD (max abs diff = 0.0, atol 1e-12).
...
## Self-Check: PASSED
- `revision/run_recover_canonical.py` — FOUND
- `revision/results/canonical_recovery.json` — FOUND (param_count 55, epoch 1969, native_pipeline B)
- `revision/results/canonical_config_lock.json` — FOUND (locked iqp_sel_55, checkpoint sha256)
- `revision/core/models/quantum.py` — FOUND (modified, default path byte-frozen)
