...
provides:
  - revision/run_distribution_emd.py (NEW top-level emitter, 50-bin histogram-density Wasserstein)
  - revision/results/distribution_emd.json (NEW aggregator JSON; 9 models × 5 seeds × 2 scales; auto-walked into v2.1 gate corpus)
  - revision/results/figures/qq_overlay.{png,pdf,json} (NEW primary discriminating figure; render_only=false)
  - revision/docs/reconciliation_note.md (3-column comparable-variants table appended; C-3 disclosure extended)
  - revision/docs/reviewer_response.md (## Marginal-convergence finding subsection appended)
  - revision/docs/methods_full.md (§3.x.(e) OD-marginal convergence cross-reference appended)
  - revision/docs/peer_review_remediation.md (## R2 follow-up — marginal-convergence finding subsection appended)
  - revision/docs/completeness_sweep_manifest.md (## Plan 14-15 section appended)
  - 9 per-model qq_<model>.json companions gain additive caption_note field
affects:
  - revision/run_figure_suite.py (extended with render_qq_overlay; render_qq_plot gains caption_note)
  - revision/run_model_info.py (extended with _comparable_variants_rows + 3-column table emit + C-3 paragraph extension)
key-files:
  created:
  ...
  modified:
    ...
    - revision/results/figures/qq_{ar,iqp_sel_55_repro,V1,V2,V3,vae,wgan_cnn,wgan_lstm,wgan_mlp}.json (additive caption_note)
decisions:
  - D-14-22 (`revision/core/` byte-freeze) PRESERVED across all 5 tasks — histogram-density EMD lives in top-level emitter, not core
...
|---|---|---|---|
| T1 | feat: dist-emd aggregator | `76430fd` | `revision/run_distribution_emd.py` (NEW), `revision/results/distribution_emd.json` (NEW) |
| T2 | feat: 3-column comparable table | `abf41c0` | `revision/run_model_info.py`, `revision/docs/reconciliation_note.md` |
| T3 | feat: qq_overlay figure | `36657ba` | `revision/run_figure_suite.py`, `revision/results/figures/qq_overlay.{png,pdf,json}` (NEW), 9 per-model `qq_<model>.json` (additive `caption_note`) |
| T4 | docs: reviewer-comms | `3a06894` | `revision/docs/reviewer_response.md`, `revision/docs/methods_full.md`, `revision/docs/peer_review_remediation.md` |
| T5 | docs: SUMMARY + sweep manifest | (this commit) | `revision/docs/completeness_sweep_manifest.md`, `.planning/phases/14-paper-revision-release-freeze/14-15-SUMMARY.md` |
...
- `revision/docs/completeness_sweep_manifest.md#plan-14-15` — full cross-plan deliverable register (this plan's section appended)
- `revision/docs/reconciliation_note.md#emd-comparable-across-metric-variants-matched-2000ep-budget` — the 3-column comparable-variants table
- `revision/results/figures/qq_overlay.png` — the single discriminating figure
- `revision/docs/reviewer_response.md#marginal-convergence-finding-post-r2-investigation` — reviewer-facing communication

## Verification (10-point checklist)

1. ✅ `revision/run_distribution_emd.py` exists and is executable; emits `distribution_emd.json` with 9-model × 5-seed rows + per-(model, scale) aggregates (90 rows, 18 aggregates, `n=5` per aggregate, `headline_present=false` because `iqp_sel_55_headline/samples.npy` is not on disk in this corpus).
...
10. ✅ `git diff` against pre-14-15 main shows zero changes to `revision/core/` (D-14-22 preserved): `git diff --stat <pre-14-15-sha> -- revision/core/` outputs nothing across all 5 tasks (verified at each task's `<verify>` gate via `[ -z "$(git diff --stat revision/core/)" ]`).
...
| `revision/docs/reviewer_response.md` | PASS | 49 (up from 32 pre-14-15) |
| `revision/docs/reconciliation_note.md` | PASS | 54 (up from 36 pre-14-15) |
| `revision/docs/methods_full.md` | PASS | 75 (up from 64 pre-14-15) |
| `revision/docs/circuit_atlas.md` | PASS | 18 |
| `revision/docs/completeness_sweep_manifest.md` | PASS | 46 (up from 27 pre-14-15) |
| `revision/docs/training_protocol.md` | PASS | 18 |
| `revision/docs/dataset_stats.md` | PASS | 5 |
| `revision/docs/peer_review_remediation.md` | PASS | 58 (up from 31 pre-14-15) |
...
- ✅ `revision/run_distribution_emd.py` exists (NEW top-level emitter).
- ✅ `revision/results/distribution_emd.json` exists with 90 rows + 18 aggregates.
- ✅ `revision/results/figures/qq_overlay.{png,pdf,json}` exist (`render_only: false`).
- ✅ 9 per-model `qq_<model>.json` carry the additive `caption_note` field.
- ✅ `revision/docs/reconciliation_note.md` carries both tables + cross-reference + C-3 extension.
- ✅ `revision/docs/reviewer_response.md` carries `## Marginal-convergence finding (post-r2 investigation)`.
- ✅ `revision/docs/methods_full.md` carries §3.x.(e) OD-marginal convergence cross-reference.
- ✅ `revision/docs/peer_review_remediation.md` carries `## R2 follow-up — marginal-convergence finding (Plan 14-15)`.
- ✅ `revision/docs/completeness_sweep_manifest.md` carries `## Plan 14-15` section.
...
- ✅ `revision/core/` byte-untouched across all 5 tasks (D-14-22 preserved).
