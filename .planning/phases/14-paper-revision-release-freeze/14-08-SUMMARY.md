...
provides:
  - "revision/run_matched2000_dualscale.py — eval-only matched-2000ep dual-scale aggregator: re-emits EMD + 4 moments + ACF lags 0-9 + DTW at BOTH OD and log_return scales for all 9 matched-2000ep models from the 45 frozen samples.npy bundles, plus the frozen-checkpoint headline as a DISTINCT row-set (D-14-10), with explicit-raise data_hash gate and loud-fail on missing samples"
  - "revision/results/matched2000_dualscale.json — the single source of truth for matched-2000ep dual-scale numbers: 2576 long-form rows[] + 560 (model,scale,metric) aggregates[], data_hash=91e447d4624e25b3, the 10th row-set is model_kind=frozen_checkpoint_headline (n_seeds=1, source=frozen_checkpoint_epoch_1969)"
  - "revision/results/figures/matched2000_dualscale_sidebyside.{png,pdf,json} — render-only side-by-side dual-scale figure (3 metric panels x 2 scales = 6 panels; frozen headline as distinct dashed reference line + black diamond marker; companion JSON records every plotted tuple + source_artifact path)"
  - "revision/results/figures/matched2000_dualscale_comparison.md — copy-paste comparison-table doc rendered via _fmt() entirely from matched2000_dualscale.json; 167 distinct literals all resolve through the existing verify_number_provenance.py gate (no gate edit)"
...
tech-stack:
  added: []
  patterns:
    - "Eval-only re-aggregation over frozen samples.npy bundles: NO retraining, NO resampling — verbatim Pipeline-B reconstruct_od (the seed*7919+1 od_start draw is load-bearing) + revision.core.eval ONLY (D-11-10). git diff --stat revision/core/ stays empty across the whole plan"
    ...
    - "Number-provenance gate (verify_number_provenance.py) auto-covers the new dual-scale JSON via its revision/results/*.json rglob — comparison.md passes the existing gate UNMODIFIED (167/167 literals resolve)"

key-files:
  created:
    - revision/run_matched2000_dualscale.py  # 625 lines (Task 1, recovered)
    - revision/results/matched2000_dualscale.json  # 28270 lines / ~697 KB (Task 1, recovered)
    - revision/results/figures/matched2000_dualscale_sidebyside.png  # Task 2
    - revision/results/figures/matched2000_dualscale_sidebyside.pdf  # Task 2
    - revision/results/figures/matched2000_dualscale_sidebyside.json  # Task 2 (60 plotted tuples + source_artifact)
    - revision/results/figures/matched2000_dualscale_comparison.md  # Task 2 (passes verify_number_provenance.py)
  modified:
    - revision/run_figure_suite.py  # Task 2: +305 lines (new dual-scale routines + main() wiring)
...
- `revision/run_matched2000_dualscale.py` (625 lines) — eval-only aggregator copied end-to-end from `run_dualscale_fidelity.py` with ONLY the four matched-2000ep-required changes:
  1. `_run_base`/`_resolve_run_dir` resolves `revision/results/matched2000/runs/<model>/<seed>` and **raises `FileNotFoundError`** with a "no retraining/resampling — D-14-08" message on any absent `samples.npy` / `inverse_kwargs.npz` (loud-fail, never a silent partial aggregate — T-14-14).
...
- `revision/results/matched2000_dualscale.json` (697 KB / 28270 lines) — the single source of truth for these numbers:
...
- **Companion JSON** records every plotted (model_kind, scale, metric_name, mean, std, n_seeds, source) tuple — 60 tuples (3 metrics × 2 scales × 10 entrants) — plus:
  - `source_artifact = "revision/results/matched2000_dualscale.json"` (figure↔data provenance, T-14-17)
...
- **Number-provenance gate**: `revision/verify_number_provenance.py` **PASSES on the comparison.md** with **167 distinct literals all resolving** to `matched2000_dualscale.json` — and the gate file was **NOT modified** (the gate's existing `revision/results/*.json` rglob auto-covers the new artifact). `git diff --stat revision/verify_number_provenance.py` is empty.

### Verification (plan verify command — verbatim PASS)

- `./qgan_env/bin/python revision/run_figure_suite.py` → RUN OK (77 PNG total, up from 76 — the new sidebyside figure added; existing 76 figure stems unchanged in content though the PDFs re-render with new timestamps, see Issues Encountered)
...
- `./qgan_env/bin/python revision/verify_number_provenance.py --target revision/results/figures/matched2000_dualscale_comparison.md` → **PASS** (167/167 literals resolve)
...
- `git diff --stat revision/core/` → empty (eval module untouched across the whole plan)
...
- `revision/run_matched2000_dualscale.py` — 625-line eval-only aggregator (Task 1, recovered)
- `revision/results/matched2000_dualscale.json` — 697 KB / 28270-line dual-scale JSON; 2576 rows + 560 aggregates; data_hash=`91e447d4624e25b3` (Task 1, recovered)
- `revision/run_figure_suite.py` — +305 lines: the two new dual-scale render routines + `main()` wiring (Task 2)
- `revision/results/figures/matched2000_dualscale_sidebyside.png` — 230 KB, 3×2-panel side-by-side figure (Task 2)
- `revision/results/figures/matched2000_dualscale_sidebyside.pdf` — 30 KB vector variant (Task 2)
- `revision/results/figures/matched2000_dualscale_sidebyside.json` — 571-line companion (60 plotted tuples + source_artifact + conflation_guard) (Task 2)
- `revision/results/figures/matched2000_dualscale_comparison.md` — 41-line copy-paste comparison table, 167 literals (Task 2)
...
- **Issue:** The prior executor produced Task 1's artifacts (`revision/run_matched2000_dualscale.py` and `revision/results/matched2000_dualscale.json`) byte-identical to the verified contents but hit an API socket error post-emission and never executed the per-task commit step. The worktree was left clean-content / unstaged.
...
- **Files affected:** `revision/run_matched2000_dualscale.py`, `revision/results/matched2000_dualscale.json` (created via recovery commit `b3235d9`).
...
- **T-14-15** (hand-typed / re-implemented metric number) — mitigated by `revision.core.eval` import-only metric math (verbatim `_od_scale_rows`/`_log_return_rows`); `git diff --stat revision/core/` empty; comparison.md gated by `verify_number_provenance.py` (167/167 literals resolve).
...
- `revision/run_matched2000_dualscale.py` — FOUND (625 lines, on the working tree from commit `b3235d9`)
- `revision/results/matched2000_dualscale.json` — FOUND (697 KB / 28270 lines, data_hash=`91e447d4624e25b3`, 2576 rows, 560 aggregates, on commit `b3235d9`)
- `revision/run_figure_suite.py` — FOUND (1139 lines post-edit, includes new dual-scale routines + `main()` wiring)
- `revision/results/figures/matched2000_dualscale_sidebyside.png` — FOUND (3×2-panel figure, 230 KB)
- `revision/results/figures/matched2000_dualscale_sidebyside.pdf` — FOUND (30 KB)
- `revision/results/figures/matched2000_dualscale_sidebyside.json` — FOUND (60 plotted tuples + source_artifact + conflation_guard)
- `revision/results/figures/matched2000_dualscale_comparison.md` — FOUND (41 lines, 167 literals)
- Plan verify gate (full sequence) — PASS: `run_figure_suite.py` runs; all 4 Task-2 artifacts present; `verify_number_provenance.py --target ...comparison.md` returns "PASS — 167 distinct numeric literal(s) all resolve to revision/results/*.json"; `matplotlib.use("Agg")` grep present; `FileNotFoundError|render-only` grep present.
...
- `git diff --stat revision/core/` — empty (eval module untouched across whole plan).
- `git diff --stat revision/verify_number_provenance.py` — empty (gate unmodified).
