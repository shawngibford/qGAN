...
- `run_matched2000_dualscale.py` (625 lines) — eval-only aggregator copied end-to-end from `scripts/run_dualscale_fidelity.py` with ONLY the four matched-2000ep-required changes:
  ...
  3. Metric math is `revision.core.eval` IMPORT-ONLY: `compute_emd`/`compute_moments`/`compute_acf`/`compute_dtw` are reused UNCHANGED; the VERBATIM `_od_scale_rows`/`_log_return_rows` bodies and `build_real_references` are copied from `scripts/run_dualscale_fidelity.py` so every metric reconciles with the audited dualscale recipe (D-11-10 / T-14-15).
...
- **No-recompute discipline**: figure renders by reading the JSON only; no metric is recomputed during render. `grep '.fit(/def train_/model.sample('` on the entire `scripts/run_figure_suite.py` returns no matches (render-only end-to-end).
- **Number-provenance gate**: `scripts/verify_number_provenance.py` **PASSES on the comparison.md** with **167 distinct literals all resolving** to `matched2000_dualscale.json` — and the gate file was **NOT modified** (the gate's existing `results/*.json` rglob auto-covers the new artifact). `git diff --stat verify_number_provenance.py` is empty.

### Verification (plan verify command — verbatim PASS)

- `./qgan_env/bin/python scripts/run_figure_suite.py` → RUN OK (77 PNG total, up from 76 — the new sidebyside figure added; existing 76 figure stems unchanged in content though the PDFs re-render with new timestamps, see Issues Encountered)
...
- `./qgan_env/bin/python scripts/verify_number_provenance.py --target figures/matched2000_dualscale_comparison.md` → **PASS** (167/167 literals resolve)
- `grep 'matplotlib.use("Agg")'` in `scripts/run_figure_suite.py` → PASS (headless before pyplot)
- `grep 'FileNotFoundError|render-only'` in `scripts/run_figure_suite.py` → PASS (loud-fail wired)
...
- No training/sampling/checkpoint-reload path in either Task-1's aggregator or Task-2's renderer: `grep -E '\.fit\(|def train_|model\.sample\('` returns no matches in `run_matched2000_dualscale.py` or in the new `scripts/run_figure_suite.py` routines.
...
- `scripts/run_figure_suite.py` — +305 lines: the two new dual-scale render routines + `main()` wiring (Task 2)
...
- **T-14-15** (hand-typed / re-implemented metric number) — mitigated by `revision.core.eval` import-only metric math (verbatim `_od_scale_rows`/`_log_return_rows`); `git diff --stat core/` empty; comparison.md gated by `scripts/verify_number_provenance.py` (167/167 literals resolve).
...
- `scripts/run_figure_suite.py` — FOUND (1139 lines post-edit, includes new dual-scale routines + `main()` wiring)
...
- Plan verify gate (full sequence) — PASS: `scripts/run_figure_suite.py` runs; all 4 Task-2 artifacts present; `verify_number_provenance.py --target ...comparison.md` returns "PASS — 167 distinct numeric literal(s) all resolve to results/*.json"; `matplotlib.use("Agg")` grep present; `FileNotFoundError|render-only` grep present.
