...
provides:
  - "revision/run_figure_suite.py — render-only per-model + cross-model + introspection figure generator (loud-fail on missing companion, dual PNG+PDF + same-stem reproducibility JSON)"
  - "revision/results/figures/ — 76 PNG figures, each with matching PDF + JSON (>= the verified 16-figure canonical bar)"
...
key-files:
  ...
  modified:
    - revision/results/figures/ (76 PNG + 76 PDF + 76 JSON triples; 3 prior introspection PDFs re-rendered byte-identically in content)
...
- **Completed:** 2026-05-19
- **Tasks:** 1
- **Files modified:** 1 created + the `revision/results/figures/` suite (228 figure files: 76 PNG / 76 PDF / 76 JSON)

## Accomplishments

### Task 1 — Render-only per-model + cross-model + analysis figure suite
- `revision/run_figure_suite.py` copies the `run_introspect_figures.py` shape end-to-end: headless `matplotlib.use("Agg")` BEFORE pyplot, the `_require`/`_load_json` loud-fail (`FileNotFoundError` with a render-only message — never a silent partial figure), `_save()` writing `<stem>.png` + `<stem>.pdf` at `dpi=150, bbox_inches="tight"` + `plt.close` PLUS a same-stem `<stem>.json` reproducibility companion, the `_find_repo_root()` resolver, the `argparse --figures-dir` default of `revision/results/figures`, and print-every-written-path.
- Added the verbatim `_bootstrap_repo_on_path()` from `run_dualscale_fidelity.py:69-83` so the plan's bare-script verify command (`./qgan_env/bin/python revision/run_figure_suite.py`) works as well as `-m revision.run_figure_suite`.
...
- `./qgan_env/bin/python revision/run_figure_suite.py` → RUN OK
...
- `revision/run_figure_suite.py` — render-only PNG+PDF+JSON figure suite generator (834 lines)
- `revision/results/figures/` — 76 PNG / 76 PDF / 76 JSON figure triples (per-model × 9 models × {6–8 types}, 3 cross-model, 3 introspection); the 3 prior introspection PDFs re-rendered byte-identically in content from the unchanged companion JSON
...
- **Found during:** Task 1 (first run via the plan's verify command `./qgan_env/bin/python revision/run_figure_suite.py`).
...
- **Fix:** Added the **verbatim** `_bootstrap_repo_on_path()` from the canonical peer `run_dualscale_fidelity.py:69-83` (walk up to the dir holding `revision/core/preprocessing.py` and prepend to `sys.path`) before the `revision.*` imports. Both `-m` and bare-script invocation now work; no behavior change beyond import resolution.
- **Files modified:** `revision/run_figure_suite.py`
...
## Issues Encountered
- **`qgan_env` absent in worktree:** `qgan_env` is gitignored and lives in the main checkout (Plan 01/02 precedent). Resolved by the established `ln -s /…/qGAN/qgan_env qgan_env` symlink (already in `.gitignore`, never committed); the script's repo-root resolver writes figures into the worktree's `revision/results/figures/`.
...
## Self-Check: PASSED
- `revision/run_figure_suite.py` — FOUND (834 lines, `matplotlib.use("Agg")` before pyplot, `_require`/`_load_json` FileNotFoundError loud-fail, dual `savefig` + JSON companion, repo-root bootstrap, argparse `--figures-dir`)
- `revision/results/figures/*.png` — FOUND (76 PNG; every PNG has a matching PDF + JSON)
