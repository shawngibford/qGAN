...
venv `/tmp/qgan_env_r4` built from `requirements-pinned.txt`
...
resolves to a `results/*.json` value (schema v2.1, negative-sign-aware
...
- **data_hash:** All **27** `results/**.json` files carrying a `data_hash`
...
`core/eval.py:84` imports `fastdtw` (required by `compute_dtw`, exercised by
`run_matched2000_dualscale.py`), but `fastdtw` is absent from
`requirements-pinned.txt` and from `framework_versions.json#packages`. A
...
documented `pip install -r requirements-pinned.txt` recipe in
