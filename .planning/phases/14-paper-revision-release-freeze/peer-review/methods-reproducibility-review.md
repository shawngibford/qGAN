...
**Target documents:**
- `revision/docs/methods_full.md` (7 sections + provenance footer)
- `revision/docs/reviewer_response.md`
- Supporting JSON in `revision/results/*.json`
...
- Actual code (`revision/run_baselines.py:315`): `recon = torch.nn.functional.mse_loss(x_hat, x)`.
...
| lr_critic | 1.8046e-05 | `revision/core/__init__.py:13` | OK |
| lr_generator | 6.9173e-05 | `revision/core/__init__.py:14` | OK |
| n_critic | 9 | `revision/core/__init__.py:11` and `training.py:218` default | OK |
| lambda_gp | 2.16 | `revision/core/__init__.py:12` and `training.py:219` default | OK |
| batch_size | 12 | `revision/core/__init__.py:21` | OK |
| epochs | 2000 | `revision/core/__init__.py:20`, `MATCHED_EPOCHS=2000` in `run_matched2000.py:112` | OK |
...
VAE/AR special-casing (separate Adam lr=1e-3 for VAE, closed-form lstsq for AR) is explicit (§3 paragraph + §2.i/§2.j). Verified in `revision/run_baselines.py:285-358` (VAE) and `revision/run_baselines.py:380-409` (AR).
...
- `revision/core/models/classical.py:78` — `torch.randn(self._n, dtype=torch.float32) * _INIT_SCALE` ✓
- `revision/core/models/critic.py:67` — `self.net = self.net.double()` ✓
- `revision/core/training.py:268` — `compute_dtype = torch.float32 if device.type == "mps" else torch.float64` ✓
- `revision/core/training.py:347` — `generated_samples = generated_samples.to(compute_dtype) * 0.1` ✓
...
- The rerun command template in §5.2 IS sliced from `revision/run_matched2000.py:1-69` (the module docstring ends at line 69; methods doc says "lines 1-80" but the docstring is actually only 1-69 — minor LOW-severity drift, the template content itself is verbatim).

### §6 Contradictions — VERIFIED
- (a) `default_75` vs `iqp_sel_55`: explanation matches `default_75_config_lock.json` (num_layers=4, RX+RY, n_params=75) vs `canonical_config_lock.json` (num_layers=3, RX_only, n_params=55, checkpoint_epoch=1969). Code references `revision/core/__init__.py:17-18` (`NUM_QUBITS=5`, `NUM_LAYERS=4`) which is the `default_75` baseline ✓.
...
- Ran `./qgan_env/bin/python revision/verify_number_provenance.py --target revision/docs/methods_full.md` → **PASS — 57 distinct numeric literals all resolve to revision/results/*.json**.
...
**Fix:** ship a second file `revision/requirements-lock.txt` or `revision/requirements-pinned.txt` with `==` pins generated from `pip freeze` or matching `framework_versions.json`.
...
```
**Status:** **BLOCKER-2**. The headline result (`iqp_sel_55_headline`, §2.a, "frozen-checkpoint paper headline") relies on `best_checkpoint.pt` which is .gitignored (verified via `.gitignore:37` `*.pt`, and via the explicit comments in `revision/run_canonical_headline.py:34` and `revision/run_recover_canonical.py:43`: "`best_checkpoint.pt` and `qgan_env` are .gitignored, so in a git…"). `revision/run_canonical_headline.py:380-383` even asserts a hard sha256 lock on the checkpoint file. **A fresh clone cannot reproduce the headline number** because there is no committed source for the trained 1969-epoch checkpoint.

**Mitigation in code:** `revision/run_recover_canonical.py` exists and has logic to search for the checkpoint, but the file must come from outside the repo. The methods doc doesn't surface this — §2.a says "Checkpoint epoch | 1969 | `canonical_config_lock.json` checkpoint_epoch" without telling the reader the checkpoint file itself is not in version control.

**Fix:** either (a) commit `best_checkpoint.pt` via git-lfs / a release artifact and link from `methods_full.md §2.a`, OR (b) explicitly tell readers in §2.a that the headline can be regenerated via `revision/run_canonical_headline.py` from `best_checkpoint.pt` which must be obtained separately (and add an external location e.g. Zenodo DOI or release asset URL).

### Step 4 — Try to run a fresh 2000-epoch training
```
./qgan_env/bin/python -m revision.run_matched2000 --model V1 --seed 42 --epochs 2000
```
**Status:** PASS (assuming the pinned env from Step 2 was sorted). The `argparse` in `revision/run_matched2000.py:807-840` accepts `--model`, `--seed`, `--epochs`, `--out-root`, `--csv-path`, `--accept` — matches the command in `methods_full.md §5.2` verbatim.
...
Latent/hidden/window dimensions in JSON (latent_dim=4, hidden_dim=16, window=10) match the constants in `revision/core/models/nonadversarial.py:63-65` (`LATENT_DIM=4`, `HIDDEN=16`, `WINDOW=10`).
...
The companion `revision/docs/paper_blocks_refs_methods.md` cites `\cite{wang2018esrganenhancedsuperresolutiongenerative, akkem2024comprehensive}` and `\cite{yoon2019TimeGAN}` for GAN/VAE generally but does NOT cite Gulrajani 2017 for WGAN-GP or Kingma & Welling 2013 for ELBO.
...
|---|---|---|---|
| 1 | BLOCKER | `requirements.txt` uses `>=` not `==` pins; reviewer cannot recreate the pinned environment from the repo alone | `/Users/shawngibford/dev/phd/qGAN/requirements.txt` vs `revision/results/framework_versions.json` |
| 2 | BLOCKER | `best_checkpoint.pt` (the headline frozen checkpoint) is .gitignored and not in version control; methods_full.md §2.a does not surface this | `.gitignore:37`, `revision/run_canonical_headline.py:34`, `revision/run_recover_canonical.py:43` |
| 3 | HIGH | "bit-identical training trajectories" claim is too strong; `torch.use_deterministic_algorithms(True)` is not set; results agree only to ~1e-6 on same CPU/BLAS | `revision/docs/methods_full.md:316-318` vs absence of `use_deterministic_algorithms` in `revision/core/training.py` |
| 4 | MEDIUM | VAE training uses raw `mse_loss` for reconstruction but methods doc renders the canonical Gaussian-log-likelihood ELBO LaTeX; the equation oversimplifies the actual code | `methods_full.md §2.i` LaTeX vs `revision/run_baselines.py:315` |
...
| 6 | LOW | Methods doc §5.2 says the rerun template is sliced from "lines 1-80" of `run_matched2000.py`, but the actual module docstring is lines 1-69 | `methods_full.md:322` vs `revision/run_matched2000.py:1-69` |
| 7 | LOW | `paper_blocks_refs_methods.md` lacks an explicit Kingma & Welling 2013 (VAE) citation and a Box-Jenkins / Hamilton AR(p) foundational reference, although Gulrajani 2017 IS cited in the LaTeX paper | `revision/docs/paper_blocks_refs_methods.md` |

---

## Recommended pre-submission fixes (priority order)

1. **(BLOCKER-1 fix)** Ship `revision/requirements-pinned.txt` (or rename existing requirements.txt) with `==` pins matching `framework_versions.json`. Reference it from `methods_full.md §4.1` so reviewers see it.
...
- `/Users/shawngibford/dev/phd/qGAN/revision/core/data.py`
- `/Users/shawngibford/dev/phd/qGAN/revision/core/training.py`
- `/Users/shawngibford/dev/phd/qGAN/revision/core/models/classical.py`
- `/Users/shawngibford/dev/phd/qGAN/revision/core/models/nonadversarial.py`
- `/Users/shawngibford/dev/phd/qGAN/revision/core/models/critic.py`
- `/Users/shawngibford/dev/phd/qGAN/revision/core/__init__.py`
- `/Users/shawngibford/dev/phd/qGAN/revision/run_matched2000.py`
- `/Users/shawngibford/dev/phd/qGAN/revision/run_baselines.py`
- `/Users/shawngibford/dev/phd/qGAN/revision/run_canonical_headline.py`
- `/Users/shawngibford/dev/phd/qGAN/revision/run_methods_full.py`
