...
### Task 1 — `scripts/run_classical_arch_extract.py` + `classical_architectures.json`
...
### Task 2 — `scripts/run_framework_versions.py` + `framework_versions.json`
...
### Task 3 — `scripts/run_methods_full.py` + `methods_full.json`
...
- `scripts/run_classical_arch_extract.py` — 423 lines, pure-introspection emitter
- `scripts/run_framework_versions.py` — 93 lines, importlib.metadata version pinner
- `scripts/run_methods_full.py` — 482 lines, pure aggregator + cross-artifact data_hash gate
...
- **Files modified:** `scripts/run_methods_full.py` (the `_first_lineno` and its `_citations` callsite docstring).
...
- **Files modified:** `scripts/run_framework_versions.py` (3 docstring tokens).
...
**Total deviations:** 3 auto-fixed (1 acceptance-criterion-phrasing nuance, 1 Rule-1 substring-matching bug, 1 Rule-3 docstring-grep blocker). No scope creep — every fix is internal to the 3 new scripts; no `core/` edit, no `scripts/verify_number_provenance.py` edit.
...
- **Auto-coverage extension:** the 3 new JSONs (`classical_architectures.json`, `framework_versions.json`, `methods_full.json`) are now in `scripts/verify_number_provenance.py`'s resolution corpus via the existing `results/*.json` rglob — zero verifier edit, no scope creep into the byte-frozen gate file.
...
- **T-14-34** (`scripts/verify_number_provenance.py` edited to force pass) → mitigated by Tasks 3 + 4 verify gates asserting `[ -z "$(git diff --stat verify_number_provenance.py)" ]` — empty.

No threat flags.

## Self-Check: PASSED

- `scripts/run_classical_arch_extract.py` — FOUND (423 lines, pure introspection, named_modules walk + functional-layout docstring parse + ARBaseline hand-encoded spec + drift gate)
- `scripts/run_framework_versions.py` — FOUND (93 lines, importlib.metadata.version over 6 packages, no network/installer/spawn)
- `scripts/run_methods_full.py` — FOUND (482 lines, pure aggregator, no torch/pennylane/core import, cross-artifact data_hash gate, runtime-grep'd citations, verbatim docstring slice)
...
- `scripts/verify_number_provenance.py` — BYTE-IDENTICAL (gate unmodified; `git diff --stat verify_number_provenance.py` empty)
