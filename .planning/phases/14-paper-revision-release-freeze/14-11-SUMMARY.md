...
provides:
  - "revision/run_classical_arch_extract.py — pure-introspection emitter: imports WGAN-GP/VAE/Critic ONLY to walk module.named_modules() + functional-layout docstring parse for params_pqc-carved generators + hand-encoded ARBaseline spec; emits classical_architectures.json with explicit-raise total_params drift gate against model_info.json"
  - "revision/run_framework_versions.py — pure-introspection emitter: importlib.metadata.version(...) over {pennylane, torch, numpy, scipy, matplotlib, PyYAML} + sys.version + platform; emits framework_versions.json (no network, no installer, no out-of-process spawn)"
  - "revision/run_methods_full.py — pure aggregator: consumes model_info.json + 5 config-lock JSONs + classical_architectures.json + framework_versions.json + text-only file:line greps over revision/core/training.py + verbatim docstring slice of revision/run_matched2000.py:1-80; emits methods_full.json with 5 paper-ready buckets + cross-artifact data_hash gate (explicit-raise, python -O safe)"
  - "revision/results/classical_architectures.json — per-model layer-tree JSON for wgan_mlp/cnn/lstm/vae/ar + shared_critic"
  - "revision/results/framework_versions.json — exact installed-version pin (Phase-14 runtime dependencies)"
  - "revision/results/methods_full.json — paper-ready 5-bucket Methods aggregator (1_dataset, 2_models, 3_training, 4_hardware_software, 5_reproducibility)"
  - "revision/docs/methods_full.md — paper-ready PAPER-08 + PAPER-09 Methods document (7 sections + provenance footer) PASSING verify_number_provenance.py UNMODIFIED with 57 distinct numeric literals all resolving"
...
tech-stack:
  ...
  patterns:
    ...
    - "Auto-coverage via verify_number_provenance.py's existing revision/results/*.json rglob: the 3 new JSONs are automatically in the gate's resolution corpus with ZERO verifier edit (D-14-22 byte-freeze-friendly extension; mirrors 14-09/14-10 contracts)"
...
### Task 1 — `revision/run_classical_arch_extract.py` + `classical_architectures.json`
...
### Task 2 — `revision/run_framework_versions.py` + `framework_versions.json`
...
- `revision/results/framework_versions.json` (sort_keys=True): schema `"framework-versions v1 (Phase 14 plan 14-11)"`, `python_version` from `sys.version.split()[0]`, `platform` from `platform.platform()`, `captured_at_iso` from `datetime.datetime.utcnow().isoformat() + "Z"`, `packages` dict with all 6 resolved. Captured pin: `pennylane=0.43.0`, `torch=2.9.0`, `numpy=2.3.4`, `scipy=1.16.2`, `matplotlib=3.10.7`, `PyYAML=6.0.3`, Python `3.11.14`, platform `macOS-26.0.1-arm64-arm-64bit`.

### Task 3 — `revision/run_methods_full.py` + `methods_full.json`
- **Pure aggregator (zero torch / zero pennylane / zero revision.core import):** loads `model_info.json` + the 5 config-lock JSONs (canonical / default_75 / v1 / v2 / v3) + `classical_architectures.json` (Task 1) + `framework_versions.json` (Task 2) via the loud-fail `_require` / `_load_json` `FileNotFoundError` idiom. Opens `revision/core/training.py` and `revision/run_matched2000.py` ONLY via `path.read_text()` — never imports them as Python modules. Verified no `import torch`, no `import pennylane`, no `from revision.core` anywhere in the source.
...
- **Verbatim docstring slicing:** opens `revision/run_matched2000.py`, finds the first `"""` and the second `"""`, and stores the text between them VERBATIM as a single 3862-char string in `buckets.5_reproducibility.rerun_command_template` — never paraphrased, never stripped (T-14-32 mitigation).
...
### Task 4 — `revision/docs/methods_full.md` (paper-ready Methods document)
...
- **Number-provenance gate PASS UNMODIFIED:** `./qgan_env/bin/python revision/verify_number_provenance.py --target revision/docs/methods_full.md` → **PASS, 57 distinct numeric literals all resolve to revision/results/*.json values**. `verify_number_provenance.py` itself is byte-identical to base (`git diff --stat revision/verify_number_provenance.py` empty). Auto-coverage via the gate's existing `revision/results/*.json` rglob — the 3 new JSONs (Tasks 1 / 2 / 3) are automatically in the resolution corpus with ZERO verifier edit (mirrors the 14-09 / 14-10 contract).
...
- **§6(a) contradiction resolution:** explicit paragraph citing `default_75_config_lock.json`, `canonical_config_lock.json`, `v1_config_lock.json`, `v2_config_lock.json`, `v3_config_lock.json` + `revision/core/__init__.py` (NUM_QUBITS=5, NUM_LAYERS=4) and `D-14-01` for the frozen-checkpoint provenance.
...
- **Task 1:** extractor runs idempotently (content-stable modulo `extracted_at_iso`); JSON schema check (5 model entries + shared_critic + total_params matches model_info.json) → PASS; `grep 'raise AssertionError'` PASS; `grep 'named_modules'` PASS; `! grep -qE '\.fit\(|def train_|model\.sample\(|best_checkpoint\.pt'` PASS; `git diff --stat revision/core/` empty PASS.
- **Task 2:** extractor runs; JSON schema check (6 packages + python_version + platform + captured_at_iso) → PASS; `grep 'importlib.metadata'` PASS; `! grep -qE 'subprocess|requests\.|urllib|pip install'` PASS (after deviation-#3 docstring rephrasing); `git diff --stat revision/core/` empty PASS.
- **Task 3:** aggregator runs; JSON schema check (data_hash + 5 buckets + 10 models + DISTINCT dtype fields + training.py:245 in manual_seed citation + rerun_command_template length > 200) → PASS (template length 3862); `grep 'raise AssertionError'` PASS; `! grep -qE 'import torch|import pennylane|from revision\.core'` PASS; `! grep -qE '\.fit\(|def train_|model\.sample\(|best_checkpoint\.pt'` PASS; `git diff --stat revision/core/ revision/verify_number_provenance.py` empty PASS.
- **Task 4:** doc exists; `verify_number_provenance.py --target revision/docs/methods_full.md` → PASS (57 literals resolve); substring sweep (23 required tokens) → all FOUND; `git diff --stat revision/core/ revision/verify_number_provenance.py` empty PASS.
...
- `revision/run_classical_arch_extract.py` — 423 lines, pure-introspection emitter
- `revision/run_framework_versions.py` — 93 lines, importlib.metadata version pinner
- `revision/run_methods_full.py` — 482 lines, pure aggregator + cross-artifact data_hash gate
- `revision/results/classical_architectures.json` — 5 model layer trees + shared_critic
- `revision/results/framework_versions.json` — 6-package exact pin + python_version + platform
- `revision/results/methods_full.json` — 5 paper-ready buckets + LaTeX equation strings + verbatim rerun template
- `revision/docs/methods_full.md` — 428 lines, 7 content sections + provenance footer (57 literals all resolve)
...
- **Files modified:** `revision/run_methods_full.py` (the `_first_lineno` and its `_citations` callsite docstring).
- **Verification:** re-emitted `methods_full.json`, confirmed `manual_seed: training.py:245`, `numpy_seed: training.py:246`, `random_seed: training.py:247`, `cuda_seed: training.py:249` — all matching the actual line content in `revision/core/training.py`.
...
- **Found during:** Task 2 verify gate (`! grep -qE 'subprocess|requests\.|urllib|pip install' revision/run_framework_versions.py`).
...
- **Files modified:** `revision/run_framework_versions.py` (3 docstring tokens).
...
**Total deviations:** 3 auto-fixed (1 acceptance-criterion-phrasing nuance, 1 Rule-1 substring-matching bug, 1 Rule-3 docstring-grep blocker). No scope creep — every fix is internal to the 3 new scripts; no `revision/core/` edit, no `verify_number_provenance.py` edit.

## Issues Encountered

- **`qgan_env` absent in worktree:** `qgan_env` is gitignored and lives in the main checkout. Resolved by the established `ln -s /Users/shawngibford/dev/phd/qGAN/qgan_env qgan_env` symlink (already in `.gitignore`, never committed) — same idiom as plans 14-01 / 14-02 / 14-09 / 14-10. The scripts' repo-root resolvers write artifacts into the worktree's `revision/results/` and `revision/docs/`.
...
- **PAPER-08 + PAPER-09 are now Methods-complete:** `revision/docs/methods_full.md` is the consolidated paper-ready document; every numeric literal traces to a JSON; both documented contradictions are explicitly resolved with file:line citations to BOTH involved config-lock JSONs.
- **Reusable Methods-bucket JSON delivered:** `revision/results/methods_full.json` is the single source of truth for any future LaTeX-block rendering of the Methods section (mirrors the 14-03 / 14-05 / 14-06 pattern).
- **Auto-coverage extension:** the 3 new JSONs (`classical_architectures.json`, `framework_versions.json`, `methods_full.json`) are now in `verify_number_provenance.py`'s resolution corpus via the existing `revision/results/*.json` rglob — zero verifier edit, no scope creep into the byte-frozen gate file.
...
- **T-14-30** (hand-typed numeric literal in methods_full.md) → mitigated by Task 4's `verify_number_provenance.py --target revision/docs/methods_full.md` PASS UNMODIFIED (57 literals all resolve).
...
- **T-14-33** (`revision/core/` edit slips in) → mitigated by every task's verify gate asserting `[ -z "$(git diff --stat revision/core/)" ]`. Verified after every commit — empty.
- **T-14-34** (`revision/verify_number_provenance.py` edited to force pass) → mitigated by Tasks 3 + 4 verify gates asserting `[ -z "$(git diff --stat revision/verify_number_provenance.py)" ]` — empty.

No threat flags.

## Self-Check: PASSED

- `revision/run_classical_arch_extract.py` — FOUND (423 lines, pure introspection, named_modules walk + functional-layout docstring parse + ARBaseline hand-encoded spec + drift gate)
- `revision/run_framework_versions.py` — FOUND (93 lines, importlib.metadata.version over 6 packages, no network/installer/spawn)
- `revision/run_methods_full.py` — FOUND (482 lines, pure aggregator, no torch/pennylane/core import, cross-artifact data_hash gate, runtime-grep'd citations, verbatim docstring slice)
- `revision/results/classical_architectures.json` — FOUND (5 model entries + shared_critic, schema header correct, total_params drift gate PASSED against model_info.json)
- `revision/results/framework_versions.json` — FOUND (6 packages all resolved, python_version 3.11.14, platform macOS-26.0.1-arm64-arm-64bit)
- `revision/results/methods_full.json` — FOUND (data_hash 91e447d4624e25b3, 5 buckets, 10 model entries, DISTINCT dtype fields, rerun_command_template length 3862, all 4 seed citations correct)
- `revision/docs/methods_full.md` — FOUND (7 sections + provenance footer, 428 lines, verify_number_provenance.py PASS 57 literals, 23 required substring tokens all present)
- `verify_number_provenance.py` — BYTE-IDENTICAL (gate unmodified; `git diff --stat revision/verify_number_provenance.py` empty)
- `revision/core/` — BYTE-FROZEN (`git diff --stat revision/core/` empty after all 4 tasks)
