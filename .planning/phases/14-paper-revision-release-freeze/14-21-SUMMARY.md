...
provides:
  - _wgan_unscale.py shared inverse-pipeline correction module
  ...
  - run_welch_aggregator.py: H2 parametric-equivalence acceptance gate dropped (soft-fail with rationale; preserves traceability)
  - results/lr_dtw_dominance_gap.json: new derived JSON making the supp per-seed LR-DTW gap literals provenance-traceable
affects: [14-07 (Zenodo freeze + DOI), AIChE resubmission bundle, methods_full.md, peer_review_remediation.md, future tag (v1.2.5+)]

tech-stack:
  added:
    - shared inverse-correction module pattern (_wgan_unscale.py — single source of truth for the x10 helper, imported by 7 producer files)
  patterns:
...
key-files:
  created:
    - _wgan_unscale.py
    - results/lr_dtw_dominance_gap.json
    - .planning/phases/14-paper-revision-release-freeze/.continue-here-t05.md
  modified:
    - run_matched2000_dualscale.py (line 189 + helper import)
    - run_distribution_emd.py (line 217 + helper import)
    - run_utility.py (line 170 + helper import)
    - run_timegan_scores.py (line 165 + helper import)
    - run_ansatz_comparison.py (line 159 + helper import)
    - run_canonical_headline.py (line 232 + headline-row label gating)
    - run_figure_suite.py (lines 272, 843, 3118 + helper import; mean-match drift band-aid removed at lines 859-863)
    - run_welch_aggregator.py (H2 acceptance threshold gates relaxed to soft-fail with documented R3 rationale)
...
key-decisions:
  ...
  - "Shared helper at _wgan_unscale.py (NOT inside core/): preserves the D-14-22 core/ byte-freeze while giving every producer a single source of truth for the model_kind gating set."
  ...
  - "Out-of-scope per plan: retraining; touching core/, training-time x0.1 sites, VAE/AR sample paths, or v1.2-v1.2.4 tags; closing 14-07 (Zenodo deferred to acceptance); addressing the residual mean-drift (training-side issue acknowledged in disclosure paragraph)."
...
|------|--------|-------------|
| T01 | `b367e52` | New shared helper module `_wgan_unscale.py` (gated x10 correction at the samples.npy load boundary, `_WGAN_KINDS` set of 10 model_kinds); 9 verified samples.npy load sites across 7 producer files wired through the helper; mean-match drift-correction band-aid removed from `render_reconstruction_overlay`; `core/` byte-unchanged. |
| T05 (early) | `9391b1a` | Pulled forward at T02 step 7 when `scripts/run_welch_aggregator.py` aborted on the OD-EMD H2 strong-claim threshold; structured directional-shift report at `.continue-here-t05.md` with pre/post means + ranking flips for OD-EMD, LR-EMD, OD-DTW, LR-DTW + the hard-prohibition tension on §6 #2 + 5 explicit R1-R5 user options. |
...
| T02-prep | `30a6cb4` | `scripts/run_welch_aggregator.py` strong-claim threshold gates converted from hard-abort to soft-fail with R3 rationale comment + JSON note field (Rule 4 deviation, user-authorised). |
...
| T04 | `99fef8e` | Paper literals refreshed in `main (4) copy.tex` (172 distinct literals) and `paper/supp_material.tex` (initial pass); JSON-diff manifest approach: snapshot pre-fix JSONs to `/tmp/pre_fix_jsons/`, diff cell-by-cell, grep+replace in .tex, final-verify with `scripts/verify_number_provenance.py`. Headline + Cross-Model + Per-seed LR-DTW + LR-EMD pairwise Welch tables refreshed; abstract + Plain Language Summary + Contributions list reframed per R3. New `results/lr_dtw_dominance_gap.json` adds the 5 per-seed gap literals to JSON for provenance traceability. |
...
| `paper/supp_material.tex` number-provenance gate | PASS (214 distinct literals trace to JSON) |
...
| `git diff core/` | empty (D-14-22 byte-freeze preserved) |
| `git diff` on training-time x0.1 sites | empty (run_matched2000.py, core/training.py, run_baselines.py byte-unchanged) |
| `git diff checkpoints/ results/matched2000/runs/` | empty (no retraining; sample artifacts byte-unchanged) |
...
6. ✓ `core/` byte-unchanged — helper at top-level `_wgan_unscale.py`; D-14-22 byte-freeze preserved.
...
3. **`scripts/run_welch_aggregator.py` strong-claim acceptance threshold relaxed** — `floor_welch_p_OD: 0.36` and `ceiling_abs_cohen_d_OD: 0.65` no longer hard-abort; soft-fail with documented Rule 4 rationale; threshold values preserved in `strong_claim_thresholds` for historical traceability.
...
| `fidelity_dualscale.json` (the 1000-epoch legacy driver's output) | `scripts/run_dualscale_fidelity.py` patched for code-symmetry but the JSON is NOT re-emitted — documented out-of-scope; if T04 provenance gate had raised a literal tracing only to that JSON, would have paused for user input (did not) |
