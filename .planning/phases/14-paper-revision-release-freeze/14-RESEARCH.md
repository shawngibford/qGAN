...
|------------|-------------|----------------|-----------|
| 55-param circuit reconstruction | `revision/core/models/quantum.py` (config-selectable) | `best_checkpoint.pt` (ground-truth oracle) | D-14-01/02: checkpoint tensor layout drives a deterministic decomposition; added as NON-default circuit (core default stays byte-frozen) |
| Matched-budget re-execution | `revision/run_*.py` + `*_sweep.sh` (`xargs -P2`) | `revision/results/*/sweep_status.json` | D-14-08/12/14: established resumable sweep pattern; tier-gated |
| Number/figure provenance | `revision/run_model_info.py` → `model_info.json` (NEW) | `revision/core/eval` helpers | D-14-16: docs/table render FROM JSON; no hand-typed numbers |
...
| Reviewer traceability | `revision/docs/reviewer_response.md` (NEW) | `QGAN_Review_Response_Plan.md.pdf` (comment IDs) | D-14-19: per-reviewer point-by-point rebuttal |
| Release freeze + DOI | git tag `v2.0-revision` + **manual** Zenodo deposit | `revision/docs/release.md` (NEW) | D-14-21/22: DOI minted LAST, over final numbers |
...
| matplotlib | Figure suite (PNG + PDF @ dpi=150, `bbox_inches="tight"`) | Pattern in `revision/run_introspect_figures.py` `[VERIFIED: read source]` |
...
```
  ...
  (headline, baseline_        (sensitivity grids,            clean 2000ep retrain
   comparison, tstr)           ansatz_comparison V1/V2/V3)   (NON-load-bearing,
            ...
            └──────────────┬──────────────┘
              ...
              └───────────────┬───────────────────────┘
                              ...
                              ▼
   revision/run_model_info.py ──▶ revision/results/model_info.json (data_hash)
                              │
            ┌─────────────────┼──────────────────────────┐
            ▼                 ▼                          ▼
   ...
   FROM JSON)           ≥ canonical 16-fig set)
            └─────────────────┬──────────────────────────┘
                              ▼
   PAPER-01..11 copy-paste LaTeX blocks  +  revision/docs/reviewer_response.md
   ...
   git tag v2.0-revision  ──▶  MANUAL Zenodo deposit (prereserve_doi)
                              ...
                              ──▶ revision/docs/release.md (tag SHA + DOI + steps)
```
...
```
`revision/docs/release.md` and `revision/docs/reviewer_response.md` **do not exist yet** `[VERIFIED: ls]` — both are new deliverables.
...
| Config-equivalence check | Ad-hoc shape print | Phase-8 parity-check harness model | D-14-07 hard-assert; parity harness already proven (`revision/results/parity_check.json`) |
...
| Build / freeze artifacts | `.gitignore` excludes `*.pt`, `*.pth`, `results/`, `qgan_env/` `[VERIFIED: grep .gitignore]`. D-14-21 says tag excludes `qgan_env/` and large checkpoints (referenced by hash), **includes** `data.csv` and `revision/results` JSON. Note `results/` IS gitignored today but `revision/results/` is the artifact home — confirm the tag actually captures `revision/results/*.json` (the gitignore `results/` pattern may or may not match the nested path; this must be verified, not assumed). | Planner must add an explicit step: verify `git check-ignore revision/results/*.json` is empty before tagging, else the DOI'd archive ships without its provenance JSON |
...
### Pitfall 4: `revision/results/` may be gitignored out of the frozen tag
**What goes wrong:** Tag `v2.0-revision` is cut, Zenodo archive published, but `revision/results/*.json` (the provenance backbone for success-criterion 5) is excluded because `.gitignore` line 62 is `results/`.
**Why it happens:** Broad gitignore pattern; D-14-21 explicitly *wants* the JSON included.
**How to avoid:** Add an explicit pre-tag verification task: `git check-ignore revision/results/<file>.json` must return nothing; if it matches, add a `!revision/results/` negation or force-add. Verify the `git archive` of the tag contains the JSON before the Zenodo upload.
**Warning signs:** `git status` not showing `revision/results/*.json` as tracked; `git ls-files revision/results` empty.
...
**How to avoid:** Pattern 3 grep-gate; every numeric literal in a PAPER block must resolve to a `revision/results/*.json` value at stated precision. The reconciliation note (1000ep→2000ep deltas, D-14-13) is the authoritative source for any changed number.
**Warning signs:** A PAPER block citing a metric with no adjacent `# source: revision/results/<file>.json#<path>` annotation.
...
```python
# Source: revision/results/baseline_comparison.json (read this session)
...
git rev-parse v2.0-revision          # SHA -> revision/docs/release.md
git check-ignore revision/results/baseline_comparison.json  # MUST be empty

# 2. Manual Zenodo deposit (prereserve_doi) — NOT GitHub integration
#    Web UI: New upload -> "Reserve DOI" -> copy version DOI
#    Write DOI into PAPER LaTeX blocks + revision/docs/release.md
...
| A4 | `.gitignore` `results/` pattern (line 62) may exclude `revision/results/` from the frozen tag | Pitfall 4 | High if true and unchecked — provenance JSON would be absent from the DOI'd archive; cheap to verify with `git check-ignore` |
...
- **D-14-15..17** One unified paper-ready table, every model a row (D-14-15); `revision/run_model_info.py` → `model_info.json`, docs render FROM JSON (D-14-16); full figure suite, ≥ canonical set (D-14-17).
- **D-14-18..20** `.tex` is read-only Overleaf reference; deliver copy-paste LaTeX blocks keyed to `\label`/anchor (D-14-18); `revision/docs/reviewer_response.md` per-reviewer (D-14-19); final tone deferred, but **PAPER-02 no-overclaiming is a LOCKED reviewer requirement regardless of result direction** (D-14-20).
...
| PAPER-08 | Dataset details in Methods | Source: regenerated `model_info.json` + `revision/docs/dataset_stats.md` (778 raw → 777 log-returns → 384 windows; single-campaign LUCY). Insert into §3 Methods after Photobioreactor setup (line 176). MUST render from JSON (D-14-16, success-criterion 5). |
...
- `revision/core/models/quantum.py` (param formula `q + L·3q + 2q`), `revision/core/__init__.py` (75-param default), `revision/run_introspect_figures.py` (PNG+PDF+JSON pattern)
- `revision/results/baseline_comparison.json` (long-form schema + `data_hash` + `metric_helpers` D-10-20)
