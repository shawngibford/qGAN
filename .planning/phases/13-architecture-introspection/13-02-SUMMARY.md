...
provides:
  - revision/run_ansatz.py — single (variant,seed) quantum WGAN driver (V2/V3)
  - revision/run_ansatz_sweep.sh — idempotent resumable 10-run sweep (xargs -P 2)
  - revision/run_ansatz_comparison.py — ARCH-02 dual-scale aggregator
  - tests/test_ansatz_json_schema.py — ARCH-02 schema regression
  - revision/results/ansatz_comparison.json — ARCH-02 comparison table (PENDING — see Blocker)
...
key-decisions:
  - "Pitfall-6 CPU pin lives in run_ansatz.py (driver), NOT revision/core/training.py — byte-unchanged discipline preserved and the frozen V1 reference is untouched"
...
- **Task 1 (committed):** `revision/run_ansatz.py` — single `(variant, seed)`
  ...
  D-13-01). `revision/run_ansatz_sweep.sh` — clone of
...
- **Task 2 source (written, not yet committed):**
  - `revision/run_ansatz_comparison.py` — clones the
    ...
    `revision/results/ansatz/runs/<variant>/<seed>/` bundles. Both scales
...
- **Sweep execution (in progress at halt):** launched
  `./revision/run_ansatz_sweep.sh --parallel 2`. At the point Bash access was
...
- **Issue:** `revision/core/training.py` unconditionally moves any generator
...
- **Fix:** Process-local guard in `run_ansatz._train_wgan`
  ...
  the 09.1/10 V1 reference runs. `revision/core/training.py` is NOT modified
  (byte-unchanged discipline; not in this plan's `files_modified`); the frozen
  V1 artifacts are untouched.
- **Files modified:** `revision/run_ansatz.py`
...
2. Run `python -m revision.run_ansatz_comparison` to emit
   `revision/results/ansatz_comparison.json`.
...
./revision/run_ansatz_sweep.sh --parallel 2
#    Wait until revision/results/ansatz/sweep_status.json -> all_complete: true (10 runs).
...
./qgan_env/bin/python -c "import json;d=json.load(open('revision/results/ansatz_comparison.json'));vs={v['variant'] for v in d['ansatz_variants']};assert vs=={'V1','V2','V3'},vs;assert d['rows'];print('OK',len(d['rows']),'rows')"

# 4. Commit Task 2 + this SUMMARY (stage files individually; never `git add .`,
#    never stage the qgan_env symlink; revision/results/ is gitignored so the
#    JSON/run-dirs are intentionally not committed):
git add revision/run_ansatz_comparison.py tests/test_ansatz_json_schema.py
...
Note: `revision/results/` is git-ignored (`.gitignore`), matching the
...
- Created files present: `revision/run_ansatz.py` (committed `08588c3`,
  fixed `6b91a3e`), `revision/run_ansatz_sweep.sh` (committed `08588c3`),
  `revision/run_ansatz_comparison.py` (written, uncommitted),
