...
provides:
  - revision/run_dualscale_fidelity.py — EVAL-05 scale-tagged fidelity re-emit driver
  - revision/results/fidelity_dualscale.json — 3360 long-form dual-scale rows with explicit scale field
...
key-files:
  created:
    ...
    - revision/results/fidelity_dualscale.json
  modified: []
...
key-decisions:
  ...
  - "Rule 3: sys.path bootstrap added so the documented bare-script invocation (python revision/run_dualscale_fidelity.py) works"
...
## Accomplishments
- `revision/run_dualscale_fidelity.py` (521 lines) — CLI driver patterned on `run_baselines.py`, with verbatim `reconstruct_od` + 50-baseline-config data_hash assert (Pitfall 4: no quantum grep).
- `revision/results/fidelity_dualscale.json` — 3360 rows; every row carries an explicit `scale ∈ {OD, log_return}`; full 6 model_kinds × 2 pipelines × 5 seeds OD coverage (60 runs).
...
- `revision/core/` byte-untouched across the entire plan (`git diff --stat 8a72391..HEAD -- revision/core/` empty — D-11-10 invariant).
...
- `revision/run_dualscale_fidelity.py` - EVAL-05 scale-tagged fidelity re-emit driver; verbatim `reconstruct_od`, run-dir resolver with canonical-checkout fallback, `emit_rows` scale-tagging loop reusing `revision.core.eval` helpers unchanged.
- `revision/results/fidelity_dualscale.json` - 3360 long-form dual-scale rows `{model_kind, pipeline, seed, metric_name, scale, value[, scale_na_reason]}`; top-level `schema`, `model_kinds`, `pipelines`, `seeds`, `data_hash`, `data_hash_verification`, `metric_helpers`, `rows`.
...
- **Issue:** `python revision/run_dualscale_fidelity.py` (the documented invocation) does not put the repo root on `sys.path`, so `from revision.core import ...` raised `ModuleNotFoundError`. The plan's verify command worked only because it injected `sys.path.insert(0,'.')`.
- **Fix:** Added `_bootstrap_repo_on_path()` (walks up to the dir holding `revision/core/preprocessing.py`, prepends to `sys.path`) — the same bootstrap the notebook generators use — called before the `revision.*` imports.
- **Files modified:** revision/run_dualscale_fidelity.py
- **Verification:** `./qgan_env/bin/python revision/run_dualscale_fidelity.py` runs end-to-end (~3:51, under 10-min budget); 3360 rows emitted.
- **Committed in:** c24a972 (Task 2 commit)

---

**Total deviations:** 2 auto-fixed (2 blocking, Rule 3)
**Impact on plan:** Both fixes were required to execute the plan in a worktree where `results/` is git-ignored and the script is invoked as documented. Neither alters metric math, the `reconstruct_od` contract, or `revision/core/`. No scope creep. Frozen artifacts consumed read-only (D-11-08); core untouched (D-11-10).
...
- FOUND: revision/run_dualscale_fidelity.py (521 lines, ≥150 required)
- FOUND: revision/results/fidelity_dualscale.json (660,559 bytes, 3360 rows, contains "log_return")
...
- VERIFIED: git diff --stat 8a72391..HEAD -- revision/core/ empty (D-11-10)
...
- Note for orchestrator: `revision/results/fidelity_dualscale.json` was force-added (`git add -f`) because `.gitignore` excludes `results/`, consistent with how 09.1 force-added `transform_ablation/runs/`. Baseline run bundles remain git-ignored and were read via the canonical-checkout fallback — downstream plans/CI must run where those frozen artifacts exist or git-track them.
- No blockers.
