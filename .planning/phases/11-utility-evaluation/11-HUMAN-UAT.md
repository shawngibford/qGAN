...
expected: Run `python run_dualscale_fidelity.py` from a machine that is NOT `/Users/shawngibford/dev/phd/qGAN` (or after renaming/moving the repo) without setting `QGAN_CANONICAL_REPO`. Driver should emit `fidelity_dualscale.json` with `data_hash==91e447d4624e25b3` and 3360 rows.
