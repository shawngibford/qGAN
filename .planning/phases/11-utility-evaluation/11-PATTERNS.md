...
|-------------------|------|-----------|----------------|---------------|
| `run_utility.py` (NEW) | driver | batch / transform (read frozen artifacts → TSTR + augmentation → JSON) | `run_baselines.py` | exact (driver shape) + `_build_baseline_notebook.py` (TSTR/recon logic) |
| `run_timegan_scores.py` (NEW) | driver | batch / transform (frozen artifacts → post-hoc GRU nets → JSON) | `run_baselines.py` | role-match (driver shape; nets are net-new faithful port) |
| `run_dualscale_fidelity.py` (NEW, or folded into a notebook) | driver | transform (recon → `eval.py` metrics → scale-tagged rows) | `_build_baseline_notebook.py` Cell 6 (long-form metric loop) | exact (same metric suite, adds `scale`) |
| `_build_utility_notebook.py` (OPTIONAL) | utility (notebook source generator) | file-I/O | `_build_baseline_notebook.py` | exact |
| `tests/test_utility.py` (OPTIONAL, recommended) | test | request-response (assertions) | `tests/test_classical.py` / `test_nonadversarial.py` | role-match |
| `results/tstr.json` (NEW output) | config/artifact | — | `results/baseline_comparison.json` | exact (long-form schema) |
| `results/predictive_discriminative.json` (NEW output) | config/artifact | — | `results/baseline_comparison.json` | exact (long-form schema) |
| `results/augmentation.json` (NEW output) | config/artifact | — | `results/baseline_comparison.json` | exact (long-form schema + `injection_ratio`) |
...
### `run_utility.py` (driver — EVAL-01 TSTR + EVAL-04 augmentation)

**Primary analog:** `run_baselines.py` (driver shape) + `_build_baseline_notebook.py` (recon + TSTR logic, copied verbatim).

**CLI / driver skeleton** — copy from `run_baselines.py:430-529`:
```python
...
args = ap.parse_args()
```
Note `run_baselines.py:451-453` defaults `--csv-path` to `./data.csv` (repo-root relative — RESEARCH Pitfall 6: resolve to an absolute repo-root path; recomputed hash MUST equal `91e447d4624e25b3`).

**Imports pattern** — copy from `_build_baseline_notebook.py:67-99` (the notebook Cell-2 import block; drivers use the same set, no `pandas` required):
```python
...
**Data-hash invariant** — copy from `run_baselines.py:226-234` AND the assert loop from `_build_baseline_notebook.py:124-149`:
```python
...
**Run-dir resolver + `reconstruct_od`** — copy **VERBATIM** from `_build_baseline_notebook.py:167-210` (do not "clean up" — the Pipeline-B seeded `od_start` draw is load-bearing, RESEARCH Pitfall 3):
```python
...
### `run_timegan_scores.py` (driver — EVAL-02/03 faithful TimeGAN nets)

**Primary analog:** `run_baselines.py` (driver shape). The two post-hoc nets are net-new but follow the same `torch.nn.Module` + `torch.optim.Adam(lr=1e-3)` + manual minibatch-loop idiom already used by `TSTRLiteLSTM`/`train_eval_tstr` (`_build_baseline_notebook.py:395-440`) and the VAE loop (`run_baselines.py:285-377`).
...
### `run_dualscale_fidelity.py` (driver — EVAL-05 scale-tagged re-emit)

**Primary analog:** `_build_baseline_notebook.py:234-296` (Cell-6 long-form metric loop) — copy the loop structure; do NOT add new metric math (D-11-10).
...
**Reuse-unchanged mandate (D-11-10):** `git diff core/` MUST be empty after Phase 11. All EVAL-05 logic lives in this driver, never in `core/eval.py`.

---

### `_build_utility_notebook.py` (OPTIONAL notebook source)

**Exact analog:** `_build_baseline_notebook.py` (entire file). Copy the `md()`/`code()` cell-builder helpers (`:32-43`), the `CELLS = []` accumulator, the `_find_repo_root()` + `os.chdir(REPO)` Cell-2 preamble (`:74-93`), and the final `nb = {...}; NB_PATH.write_text(json.dumps(nb, indent=1))` writer (`:625-636`). Not gitignored — kept as the canonical deterministic notebook source.

---

### `tests/test_utility.py` (OPTIONAL, recommended)

**Analog:** `tests/test_classical.py` / `tests/test_nonadversarial.py` (existing pytest convention; run via `./qgan_env/bin/python -m pytest tests/ -x -q`). Assert: every loaded `samples.npy` is `(3840,10)` float64; recomputed data-hash == `91e447d4624e25b3`; TSTR round-trip vs the Phase 10 anchor (quantum|B R² ≈ 0.994, real-only R² ≈ -13.354); augmentation partition disjointness; `git diff --stat core/` empty.
...
**Source:** `results/baseline_comparison.json` (verified: 1710 rows) + the writer at `_build_baseline_notebook.py:352-378`.
...
**Source:** `_build_baseline_notebook.py:167-210`.
**Apply to:** every driver (`run_utility.py`, `run_timegan_scores.py`, `run_dualscale_fidelity.py`).
Verified on disk: baseline run dir = `results/baselines/runs/<model>/<pipeline>/<seed>/{config.yaml,checkpoint.pt|.npz,samples.npy,metrics.json,inverse_kwargs.npz}`; quantum run dir = `results/transform_ablation/runs/<pipeline>/<seed>/` (extra `_stdout.log`/`_stderr.log`, no `data_hash` in `config.yaml`). `samples.npy` confirmed `(3840,10)` float64 in `[-1,1]`-ish window space.

### Inverse-transform helpers (reuse unchanged — D-11-10)
**Source:** `core/preprocessing.py` — `inverse_logreturns` (`:49-72`, Pipeline B), `inverse_minmax_od` (`:92-96`, Pipeline A). Round-trip ≤1e-8 (ABL-01 verified). Called only through the verbatim `reconstruct_od` wrapper; never modify `core/`.

### Fidelity metric helpers (reuse unchanged — D-11-10)
**Source:** `core/eval.py` — `compute_emd` (`:25`), `compute_moments` (`:42`), `compute_acf` (`:64`, FFT), `compute_dtw` (`:78`, fastdtw+euclidean), `compute_jsd` (`:96`), `full_metric_suite` (`:143`). v1.0/v1.1 behavioral locks (raw-sample EMD, FFT ACF, Fisher kurtosis, ddof=0 std) baked in. EVAL-05 only wraps each call with a `scale` field at the driver level.

### Sweep / parallelism guardrail (only if a sweep wrapper is added)
**Source:** `run_baselines_sweep.sh`. Phase 11 work is fast aggregation over frozen artifacts (<15 min, RESEARCH A5) so a sweep is likely unnecessary; if added, copy the `is_complete`/`update_status`/atomic-`os.rename` status pattern and the `xargs -P 2` (NEVER `multiprocessing.Pool`) idiom (`:174-184`, `:401-419`). Venv-binary invocation pattern: `./qgan_env/bin/python` (`:97-107`).
...
**Analog search scope:** `revision/`, `core/`, `results/`, `results/baselines/runs/`, `results/transform_ablation/runs/`
