...
provides:
  - "run_timegan_scores.py — discriminative_score driven by a single explicit np.random.default_rng(seed) for splits + minibatches, with an enforced logits/labels shape contract"
...
- **Completed:** 2026-05-18
- **Tasks:** 1
- **Files modified:** 1 (`run_timegan_scores.py`)
...
- **Untouched:** `DiscriminativeGRU`, loss/optimizer, `iters`/`bs` defaults, the `abs(0.5 - test_accuracy)` return formula, and `core/` — all unchanged. The recorded JSON `seed` metadata value is unchanged (only RNG plumbing changed).
- **Invariants held:** `git diff --stat -- core/` empty; `predictive_discriminative.json` `data_hash` still `91e447d4624e25b3`; `pytest tests/test_timegan_scores.py -q` → 4 passed; `pytest tests/ -q` → 22 passed.
...
- `run_timegan_scores.py` - `discriminative_score`: single explicit `np.random.default_rng(seed)` for splits + minibatches; `_split` takes the Generator; documented load-bearing split order; `logits.squeeze(-1)` + shape assertion before accuracy.
...
- `pytest tests/test_timegan_scores.py -q` → 4 passed ✓
