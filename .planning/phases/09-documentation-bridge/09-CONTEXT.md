...
1. `revision/docs/training_protocol.md` — Methods-section content covering N_CRITIC, λ, optimizer, both LRs, epochs, early-stopping rule, seeds, shot/analytic distinction (DOC-01).
2. `revision/docs/dataset_stats.md` — dataset characterization: raw OD count, rolling-window count, split convention, campaign count (DOC-02).
3. Differentiable `inverse_transform` in `revision/core/data.py` — log-return + Lambert W back-transform with autograd-flowing gradients, verified ≤1e-8 round-trip (EVAL-06).

**Plus minimal scaffolding for Phase 09.1:** a new `revision/core/preprocessing.py` exposing the `forward_X`/`inverse_X` contract for all three ablation pipelines, with only the Lambert-W pair fully implemented in Phase 9. The other four entries are `NotImplementedError` stubs reserved for Phase 09.1.

**In scope:**
- Implement differentiable Lambert W as in-place replacement of `inverse_lambert_w_transform` in `revision/core/data.py`.
- Add `revision/core/preprocessing.py` skeleton with `forward_logreturns`/`inverse_logreturns`, `forward_lambert`/`inverse_lambert`, `forward_minmax_od`/`inverse_minmax_od` signatures. Lambert pair fully implemented; other four raise `NotImplementedError("Phase 09.1")`.
...
- Write `revision/docs/training_protocol.md` and `revision/docs/dataset_stats.md` in hybrid format (tables + 1-paragraph prose justifications).
...
- **D-04: Round-trip verification covers BOTH synthetic and real inputs.** A single test asserts `max_abs_error(inverse(forward(x)), x) ≤ 1e-8` on (a) a `torch.randn(776, dtype=float64)` synthetic tensor and (b) the full real `log_delta` tensor (776 elements). Synthetic = decoupled correctness; real = data-path correctness.
...
- **D-09: Numbers traceable to `revision/core/__init__.py`.** Every constant in `training_protocol.md` (N_CRITIC, λ, LRs, NUM_EPOCHS, BATCH_SIZE, NOISE_HIGH, etc.) is sourced from `revision/core/__init__.py`. Doc cites the source file once at the top so a future hyperparameter change has a single place to update.
...
- `revision/core/data.py` — current `inverse_lambert_w_transform` lives here (lines 68–87); `lambert_w_transform` (forward) at lines 90–104; both float64-promoted
- `revision/core/__init__.py` — source of truth for all hyperparameter constants; training_protocol.md cites these
- `revision/core/eval.py` — EMD definition (raw samples, not histograms — v1.0 lock); training_protocol.md references this for early-stopping rule
...
### Reusable Assets
- `revision/core/data.py::inverse_lambert_w_transform` (lines 68–87) — current scipy-based implementation. Forward result shape, dtype (float64), and sign-handling behavior must be preserved exactly; only the autograd path is changed.
- `revision/core/data.py::lambert_w_transform` (lines 90–104) — forward (Gaussian → heavy-tail). Already differentiable (pure torch ops). No changes needed; preprocessing.py's `forward_lambert` re-exports this.
- `revision/core/data.py::load_and_preprocess` (lines 187–256) — full v1.1 pipeline; emits `delta`, `mu`, `sigma`, `transformed_norm_log_delta` artifacts that the differentiable inverse must accept unchanged.
- `revision/core/__init__.py` — central constants module (N_CRITIC=9, LAMBDA=2.16, LR_CRITIC=1.8046e-5, LR_GENERATOR=6.9173e-5, NOISE_LOW=0.0, NOISE_HIGH=4π, etc.). training_protocol.md tables import + render these.
- `revision/01_parity_check.ipynb` — Phase 8 parity notebook; provides a template for the round-trip verification cell pattern (load → forward → inverse → assert).

### Established Patterns
- **No business logic in notebooks** — Phase 9 implementation lives in `revision/core/`; any verification cell goes in `revision/02_eval06_roundtrip.ipynb` (or extends `01_parity_check.ipynb`) and only orchestrates + asserts + writes JSON to `revision/results/`.
...
### Integration Points
- `revision/core/data.py::full_denorm_pipeline` (lines 134–162) — wraps the inverse Lambert W with the rest of the denormalization stack. This becomes the `inverse_lambert` in `preprocessing.py` (or close to it). Verify the wrapped pipeline is end-to-end differentiable after the Lambert W change.
- Phase 09.1's `revision/core/preprocessing.py` (Phase 9 creates skeleton) — the `forward_lambert`/`inverse_lambert` symbols are the primary public API surface. Phase 09.1 fills in the OD and log-returns variants.
