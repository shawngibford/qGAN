...
Instead it was preserved verbatim at five sites:
  * ``run_matched2000.py:281``  (training-time / sample-export)
  * ``core/training.py:347,381,416``  (per-WGAN-variant training loop)
  * ``run_baselines.py:205``  (WGAN sample-export)
So every WGAN-trained ``samples.npy`` on disk under
``results/matched2000/runs/<model>/<seed>/`` is stored at one-tenth
...
-------------------------------------
Per ``run_baselines.py:28`` "Pitfall 3 (RESEARCH)", VAE and AR were
...
# (run_baselines.py:28) — they were trained without the * 0.1.
