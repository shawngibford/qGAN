...
contains 45 trainable parameters. Full circuit specifications and the critic architecture
...
Time-series data were collected from a 20-liter photobioreactor (LUCY\textregistered, Synoxis Algae) designed for laboratory and pilot-scale cultivation of microalgae, see Figure \label{fig:lucy}  for the 300L configuration of the 20L version of LUCY. The system comprises six vertical borosilicate glass tubes (6~cm OD, 120~cm length) connected in parallel, with SALT technology (spiral air lift tubular circulation) for gentle mixing, controlled aeration, and programmable lighting managed by an integrated automated controller.  
...
The DTW distance between original and synthetic sequences was 0.6843 (Figure~\ref{fig:DTWD}).
...
While direct comparison across different datasets and domains is limited, the lower
DTW score suggests effective capture of temporal dynamics in the bioprocess setting (the DTW algorithm is detailed in Supplementary Algorithm~\ref{alg:dtw}).
...
\begin{figure*}
  ...
  log returns (log $\delta$), where $\delta$  is the return calculation. Shaded regions indicate 95\% confidence intervals.}
  \label{fig:acf}
\end{figure*}
