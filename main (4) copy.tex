...
- Bifurcated Empirical Finding: Under the matched protocol, quantum generators (55--135 parameters) uniformly outperform parameter-matched classical adversarial baselines (73--78 parameters) on log-return temporal structure, with concordant evidence from log-return dynamic time warping (LR-DTW 0.94--1.12 vs 1.58--7.70) and the lag-1 autocorrelation of the real series (quantum cluster -0.089 to -0.100 vs real -0.064; closest of any generator evaluated). On the optical-density marginal, no advantage is observed.
...
log-return temporal-structure axis (Dynamic Time Warping on the
...
adversarial generators. We report the structural-fidelity finding here
as the principal result and the OD-marginal finding as a scope-honest
caveat.
...
same structural-fidelity story and is best read jointly with LR-DTW
rather than as an independent metric. The real-data log-return lag-1
ACF, computed under the same matched preprocessing pipeline as the
generators, was $-0.064$ (Figure~\ref{fig:cross_model_acf_overlay},
dashed reference). Every quantum variant produced a mean lag-1 ACF
within a narrow band closer to the real reference than any other
generator: V3$=-0.0895$, iqp\_sel\_55\_repro$=-0.0949$, V2$=-0.0968$,
V1$=-0.0997$ (range $-0.09$ to $-0.10$). The classical adversarial
baselines overshot the negative lag-1 magnitude: wgan\_cnn$=-0.111$,
wgan\_mlp$=-0.142$, wgan\_lstm$=-0.242$; the AR(2) reference produced
lag-1$=-0.136$. The VAE lag-1 ACF was $-0.648$, an order of magnitude
larger in absolute value than the real-data reference and outside the
cross-model spread on this metric. The LR-DTW ranking and the lag-1
ACF closeness-to-real ranking are concordant: the quantum cluster
leads on both, the classical adversarial cluster trails on both, and
the VAE is anomalous on both. We therefore treat the two observations
as a single structural-fidelity finding rather than as two independent
metric outcomes.
...
\begin{figure}[!htbp]
  ...
  The quantum cluster (lag-1 ACF $-0.09$ to $-0.10$) is closest to
  real of any generator. Classical adversarial baselines and AR(2)
  overshoot the negative lag-1 magnitude; VAE (lag-1
  ACF $= -0.648$) is anomalous, consistent with the degenerate regime
...
and $0.034$; we disclose this rather than excluding it). We tested
...
finding.
On log-return temporal structure --- LR-DTW and lag-1 ACF read jointly as one
structural-fidelity axis --- the four quantum WGAN-GP variants
(LR-DTW $0.94$--$1.12$; lag-1 ACF $-0.09$ to $-0.10$) uniformly outperformed
the three parameter-matched classical adversarial baselines
(LR-DTW $1.58$--$6.86$; lag-1 ACF $-0.111$ to $-0.242$) and the AR(2)
reference (LR-DTW $7.70$; lag-1 $-0.136$), reproducing the real-data
lag-1 ACF of $-0.064$ more closely than any classical generator. On the
optical-density single-step marginal, the n=5 seed budget yields
Welch $p > 0.36$ and max $|\text{Cohen's } d| \leq 0.65$
