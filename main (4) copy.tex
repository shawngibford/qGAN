...
- Bifurcated Empirical Finding: Under the matched protocol, quantum generators (55--135 parameters) uniformly outperform parameter-matched classical adversarial baselines (73--78 parameters) on log-return temporal alignment, with concordant evidence from log-return dynamic time warping (LR-DTW 0.94--1.12 vs 1.58--7.70) and the lag-1 autocorrelation of the real series (quantum cluster mean $-0.0997$ to $-0.0895$ vs real $-0.064$; closer to real than any classical-baseline mean, with per-seed overlap noted). On the optical-density marginal, no advantage is observed.
...
Third, our empirical evaluation on a single real-world photobioreactor cultivation campaign produces a bifurcated finding (Section~4.1, Figures~\ref{fig:cross_model_dtw_dualscale} and~\ref{fig:cross_model_acf_overlay}). The quantum generators \emph{exceed} the parameter-matched classical adversarial baselines on the two log-return temporal-structure metrics (LR-DTW and lag-1 ACF closeness to real), \emph{match} them (statistically indistinguishable under the achievable $n=5$ power) on the OD single-step marginal, and \emph{fall short} on the log-return single-step marginal (LR-EMD). We present the QWGAN-GP as outperforming size-matched classical adversarial baselines on the log-return temporal-structure axis on this dataset, while restricting positive claims to that axis and to the laboratory-scale single-variable setting actually evaluated.
...
alignment (LR-DTW 0.94--1.12 vs 1.58--6.86) and on lag-1 autocorrelation
closeness to the real series ($-0.064$); they \emph{match} the classical
