...
`results/*.json` artifact. Any unresolved literal triggers an
...
downstream LaTeX-block plans (14-05, 14-06) are verified against:
    ...
    ./qgan_env/bin/python verify_number_provenance.py \
        --target docs/training_protocol.md
...
def _find_repo_root() -> Path:
    ...
    for cand in [p, *p.parents]:
        if (cand / "core" / "preprocessing.py").exists():
            return cand
    raise RuntimeError(
        "repo root not found (anchor file core/preprocessing.py missing)"
    )
...
def _json_corpus() -> dict[str, dict]:
    """Concatenated text + parsed object of every results/*.json
...
def verify(target: Path, manifest: bool = False) -> int:
    ...
    if unresolved:
        ...
        raise AssertionError(
            ...
            f"resolve to any results/*.json value at stated "
    ...
    print(
        ...
        f"resolve to results/*.json (number-provenance gate, "
        f"schema={_SCHEMA!r})."
    )
    if manifest:
