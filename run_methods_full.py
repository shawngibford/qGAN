...
Pure aggregator emitting ``results/methods_full.json`` — the
paper-ready Methods bucket JSON that ``docs/methods_full.md`` (Task
...
Pure aggregator constraints (D-14-16, mirrors 14-03 / 14-09 / 14-10):
  ...
  - Zero `core` package imports.
  - ``core/training.py`` is opened ONLY via ``path.read_text()`` for
    grep-based file:line citation extraction — never executed as a module.
  - ``run_matched2000.py`` is opened ONLY via ``path.read_text()`` for
...
# ─────────────────────────────────────────────────────────────────────────────
# Text-only file:line citation extractor — opens core/training.py
...
def _citations(training_src: str) -> dict[str, str]:
    """Return {label: 'core/training.py:N'} for each grep target.
    ...
    for label, pat in targets.items():
        ...
        if ln is None:
            raise AssertionError(
                ...
                f"core/training.py: label={label} pattern={pat!r} — "
        ...
        out[label] = f"core/training.py:{ln}"
    return out
...
def _arch_quantum(lock: dict, model_rec: dict | None) -> dict:
    ...
    arch: dict = {
        "source": f"results/{lock.get('_lock_relpath', '?')}",
...
def main() -> None:
    ...
    # ── Text-only citation extraction over core/training.py ──
    training_path = REPO / "core/training.py"
    ...
    matched_path = REPO / "run_matched2000.py"
    ...
    vae_obj = {
        ...
        "source_path": (
            "run_baselines.py (ELBO training loop — NOT in "
            "core/training.py which is WGAN-GP only; D-10-13)"
        ),
    ...
    def _classical_arch_block(kind: str) -> dict:
        ...
        block: dict = {
            "source": "results/classical_architectures.json",
            "class": rec.get("class"),
    ...
    bucket_3 = {
        ...
        "source": (
            "results/model_info.json (iqp_sel_55_repro row "
    ...
    bucket_4 = {
        ...
        "dtype_params": (
            ...
            "dtype=torch.float32 — core/models/classical.py:78; "
        ...
        "dtype_note": (
            ...
            "critic (core/models/critic.py:67 .double()). The two "
    ...
    bucket_5 = {
        ...
        "rerun_command_template_source": (
            "run_matched2000.py:1-69 (module docstring; preserved "
    ...
    out = {
        ...
        "data_hash": EXPECTED_DATA_HASH,
        "consumed_artifacts": {
            "results/model_info.json": hashes.get(
                "model_info.json"
            ),
            "results/canonical_config_lock.json": hashes.get(
                "canonical_config_lock.json",
                "no-data_hash (architecture lock)",
            ),
            "results/default_75_config_lock.json": hashes.get(
                "default_75_config_lock.json",
                "no-data_hash (architecture lock)",
            ),
            "results/v1_config_lock.json": hashes.get(
                "v1_config_lock.json",
                "no-data_hash (architecture lock)",
            ),
            "results/v2_config_lock.json": hashes.get(
                "v2_config_lock.json",
                "no-data_hash (architecture lock)",
            ),
            "results/v3_config_lock.json": hashes.get(
                "v3_config_lock.json",
                "no-data_hash (architecture lock)",
            ),
            "results/classical_architectures.json": (
