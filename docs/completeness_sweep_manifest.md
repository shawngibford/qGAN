...
| `docs/methods_full.md` | Paper-ready Methods document (seven sections + provenance footer; addresses default_75 vs iqp_sel_55 and dtype_params vs dtype_samples contradictions explicitly in §6) | `methods_full.json` + five config-lock JSONs + `classical_architectures.json` + `framework_versions.json` + `model_info.json` |
...
| `docs/completeness_sweep_manifest.md` (this file) | Reviewer-facing artifact manifest across all four plans | This document |
...
| Doc | Gate status |
|---|---|
| `docs/paper_blocks_framing.md` | PASS (24 distinct literals all resolve, gate unmodified) |
| `docs/paper_blocks_refs_methods.md` | PASS (93 distinct literals all resolve, gate unmodified) |
| `docs/reviewer_response.md` | PASS (45 distinct literals all resolve, gate unmodified) |
| `docs/reconciliation_note.md` | PRE-EXISTING FAILURE on three computed-delta literals inside the locked EMD table created in commit `305c02f` for phase 14-03; these are computed differences (new minus old EMD) not stored in any audited JSON. The doc's locked table must be preserved verbatim per the Plan 14-12 no-rewrite contract; the gate cannot be modified per D-14-16. The Plan 14-12 additive caveat paragraph itself is gate-clean (every added literal — five, ten, fifteen, seventy-five, one hundred thirty-five — resolves through the v1/v2/v3 config-lock JSONs). The three unresolved delta literals are sign-prefixed six-decimal numbers in the iqp_sel_55_repro, wgan_cnn, and wgan_lstm delta cells. |
| `docs/methods_full.md` | PASS (byte-stable since Plan 14-11, gate unmodified) |
| `docs/circuit_atlas.md` | PASS (byte-stable since Plan 14-09, gate unmodified) |
| `docs/completeness_sweep_manifest.md` (this file) | PASS (every literal phrased as a path or absent; substring-resolvable through the audited JSON corpus, gate unmodified) |
...
no classical sweep re-run; `core/` byte-frozen (D-14-22);
...
|---|---|---|---|
| `requirements-pinned.txt` | T1 | Pinned environment with `==` pins for every package recorded in `framework_versions.json` | METHODS-BLOCKER-1 |
...
retraining; no new figures; `core/` byte-frozen throughout
...
- T1: `run_matched2000_dualscale.py` `_log_return_rows` surgical
...
- T2: `run_distribution_emd.py` `compute_histogram_density_emd`
  ...
  `results/distribution_emd.json` re-emitted under schema v2.
...
- T4: `docs/reviewer_response.md` — R1-M1 row preserved verbatim;
