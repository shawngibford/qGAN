...
Emits ``results/welch_pairwise.json`` from the corrected per-seed
EMD aggregates in ``results/matched2000_dualscale.json`` (post-T1
R3-CR-2 fix). The JSON is the auditable anchor for the strong-claim Welch
literals asserted in ``docs/reviewer_response.md``'s
...
Top-level emitter — no imports from ``core/`` (D-14-22 byte-freeze
...
walker in ``verify_number_provenance.py`` (no gate edit; D-14-16
...
SOURCE_REL = Path("results/matched2000_dualscale.json")
OUT_REL = Path("results/welch_pairwise.json")
