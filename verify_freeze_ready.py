...
asserts ``docs/release.md`` (a 14-07 deliverable) exists.
  ...
  (a) gitignore / archive-content invariant (RESEARCH Pitfall 4)
      Every committed ``results/*.json`` provenance artifact MUST
      ...
      MUST carry the ``!results/`` negation and
  ...
  (b) number-provenance invariant (D-14-22, T-14-20)
      ...
      ``verify_number_provenance.py --target <file>``. The DOI
  ...
  (d) release-note invariant (Agent 6 HIGH-3, Plan 14-19)
      ``docs/release.md`` MUST exist. It is authored by plan
...
PAPER_BLOCKS = [
    REPO_ROOT / "docs" / "paper_blocks_framing.md",
    REPO_ROOT / "docs" / "paper_blocks_refs_methods.md",
    REPO_ROOT / "docs" / "reviewer_response.md",
]
...
def _check_ignored_json() -> list[str]:
    """Return the list of results/*.json paths git would ignore.

    Plan 14-13 Task 5 (HI-6): rglob recursive walk — matches the verifier's
    own .rglob behaviour in verify_number_provenance.py. The
    previous .glob walked only the top-level results/*.json files
    ...
    tracked_json = [
        p
        for p in _git("ls-files", "results").splitlines()
        if p.endswith(".json")
    ]
    if not tracked_json:
        raise AssertionError(
            "No results/*.json tracked in the committed tree — the "
...
def gate_d_release_md() -> None:
    """(d) docs/release.md MUST exist — it is a 14-07 must_have
    ...
    if not release_md.is_file():
        raise AssertionError(
            f"release note absent: {release_md} — docs/release.md "
...
def gate_a_gitignore_archive() -> None:
    ...
    ``results/`` exclusion + both ``!results/`` negations are
