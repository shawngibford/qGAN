...
production quantum circuits used in the manuscript:
    ...
    V1, V2, V3   — the matched-budget ansatz variants from
                   ``run_matched2000.py:118-122``
                   (V1: range/4L/75p, V2: range/8L/135p, V3: linear/4L/75p)

Two outputs:

    1. Four config-lock JSONs under ``results/``
...
``core.models.quantum.QuantumGenerator`` is imported only to
...
``core/`` byte-freeze (D-14-22) is preserved across the whole
plan (``git diff --stat core/`` stays empty).

Idiom mirrors ``run_figure_suite.py`` (the 14-04 canonical pattern):
...
Usage::
    ...
    python -m run_circuit_diagrams             # locks + diagrams
    python run_circuit_diagrams.py             # bare-script form
    python run_circuit_diagrams.py --config-locks-only
    python run_circuit_diagrams.py --diagrams-only
"""
...
from core import NUM_LAYERS, NUM_QUBITS, WINDOW_LENGTH  # noqa: E402
# Pure-aggregator import (D-14-22 safety; verified per 14-03 SUMMARY
# deviation #1): ``run_matched2000``'s top-level imports are
...
from run_matched2000 import _QUANTUM_ANSATZ  # noqa: E402
...
# Source-of-truth line numbers in run_matched2000.py for V1/V2/V3
# (the _QUANTUM_ANSATZ dict literally lives at lines 117-124).
_QUANTUM_ANSATZ_LINES = {
    "V1": "run_matched2000.py:118",
    "V2": "run_matched2000.py:120",
    "V3": "run_matched2000.py:122",
}

# default_75 derives from core constants + the default_75 branch of
# core/models/quantum.py — recorded verbatim in the source_path
# field so the regression-guard assertion has a deterministic origin.
_DEFAULT_75_SOURCE_PATH = (
    "core/__init__.py (NUM_QUBITS=5, NUM_LAYERS=4) + "
    "core/models/quantum.py (default_75 branch)"
)
...
def _final_rot_factor(circuit_id: str) -> int:
    """Mirror ``core/models/quantum.py:104`` exactly.
...
def _expected_param_count(num_qubits: int, num_layers: int,
                          circuit_id: str) -> int:
    """Mirror ``core/models/quantum.py:105-109`` exactly.
...
def _build_lock_object(
    *,
    ...
    Schema mirrored from ``results/canonical_config_lock.json``:
...
def build_config_locks(repo: Path) -> list[Path]:
    """Emit V1/V2/V3 + default_75 config-lock JSONs under results/.
...
def _draw_one(
    *,
    ...
    # at core/models/quantum.py:344. The placeholder param
    ...
    with torch.no_grad():
        ...
        # step never pulls in the heavy core.models.quantum
        # module (which transitively imports pennylane + torch — fine
        # here since we are about to use them anyway).
        from core.models.quantum import QuantumGenerator  # noqa: E402
