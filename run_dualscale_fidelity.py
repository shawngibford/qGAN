...
``results/fidelity_dualscale.json`` cleanly.

Re-emits every ``core.eval`` fidelity metric (EMD, moments, ACF, DTW)
...
fidelity math is reused UNCHANGED from ``core/eval.py`` (D-11-10);
...
``results/`` is git-ignored (``.gitignore:62``); the baseline run
bundles under ``results/baselines/runs/`` are NOT tracked and are
...
locally. ``core/`` is never touched.

Usage
-----
    python -m run_dualscale_fidelity \\
        [--out results] [--csv-path ./data.csv]

Writes ``results/fidelity_dualscale.json`` (long-form dual-scale
...
def _bootstrap_repo_on_path() -> Path:
    ...
    ``python run_dualscale_fidelity.py`` does not put the repo root
    on ``sys.path`` (only the script's own dir). Walk up to the dir holding
    ``core/preprocessing.py`` and prepend it (same bootstrap the
...
def _run_base(model_kind: str, pipeline: str, seed: int) -> Path:
    ...
    if model_kind == "quantum":
        rel = Path(f"results/transform_ablation/runs/{pipeline}/{seed}")
    else:
        rel = Path(
            f"results/baselines/runs/{model_kind}/{pipeline}/{seed}"
        )
...
def reconstruct_od(model_kind: str, pipeline: str, seed: int,
    ...
    Copied VERBATIM from ``_build_baseline_notebook.py:175-210``
...
def main() -> None:
    """CLI entry point — emit results/fidelity_dualscale.json."""
    ap = argparse.ArgumentParser(
        description=(
            "Phase 11 EVAL-05 scale-tagged fidelity re-emit "
            "(reuses core.eval UNCHANGED — D-11-10)."
        )
    )
    ap.add_argument(
...
def emit_rows(real_OD_flat, real_windowed_OD, real_log_delta) -> list:
    ...
    ``core.eval`` metric on ``scale="OD"``, then the same metric set
