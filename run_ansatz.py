...
Structural clone of ``run_baselines.py``'s WGAN branch (same windowed
...
ARCH-01 ansatz variants (Phase 13 CONTEXT decisions):
    D-13-01 — V1 (depth-4, range, 75 params) is NOT a sweep target here; its
              ...
              ``results/transform_ablation/runs/B/{42..46}`` (no
    ...
    D-10-20/24 — NO new metric helpers here; per-run fidelity metrics are
              computed by the Wave-2 aggregator via ``core.eval``.
...
Usage
-----
    python -m run_ansatz --variant {V2|V3} --seed N [--epochs M] \\
        [--out-root results/ansatz] [--csv-path ./data.csv]
...
from core.models.quantum import QuantumGenerator  # <- swapped (D-13-01)
from core.models.critic import Critic
from core.preprocessing import forward_logreturns
...
def _force_cpu_for_quantum():
    ...
    it does NOT modify core/training.py (byte-unchanged discipline)
